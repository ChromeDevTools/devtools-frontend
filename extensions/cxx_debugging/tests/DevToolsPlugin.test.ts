...
describe('DevToolsPlugin', () => {
  describe('addRawModule', () => {
    const expectedSources = [
      makeURL('/build/tests/inputs/hello.c'),
      makeURL('/build/tests/inputs/printf.h'),
    ];
    it('does not race with removeRawModule', async () => {
      const plugin = await createPlugin(
          new TestHostInterface(),
          new ResourceLoader(),
      );
      const sources1Promise = plugin.addRawModule('0', '', {
        url: makeURL('/build/tests/inputs/hello.s.wasm'),
      });
      ...
      const sources2Promise = plugin.addRawModule('0', '', {
        url: makeURL('/build/tests/inputs/hello.s.wasm'),
      });
    ...
    it('does not try to create module file names that contain /', async () => {
      ...
      const sources = await plugin.addRawModule('?ü', '', {
        url: makeURL('/build/tests/inputs/hello.s.wasm'),
      });
    ...
    it('reports missing module', async () => {
      ...
      sinon.assert.calledOnceWithExactly(spy, url, {
        success: false,
        errorMessage: `NotFoundError: Unable to load debug symbols from \'${url}\' (Not Found)`,
      });
    });
    ...
    it('reports loaded module and potentially missing dwp', async () => {
      ...
      sinon.assert.calledWith(spy, dwpUrl, {
        success: false,
        errorMessage: 'Failed to fetch dwp file: Not Found',
      });
    });
    ...
    it('reports loaded dwos', async () => {
      ...
      const plugin = await WorkerPlugin.create(
          defaultConfig.moduleConfigurations,
          defaultConfig.logPluginApiCalls,
      );
    ...
    it('reports missing dwos', async () => {
      ...
      const helloFileURL = makeURL(
          '/build/tests/inputs/hello-split-missing-dwo.c',
      );
      ...
      const helloDwoURL = makeURL(
          '/build/tests/inputs/hello-split-missing-dwo.dwo',
      );
      ...
      sinon.assert.calledWith(spy, helloDwoURL, {
        success: false,
        errorMessage: `Couldn't load ${helloDwoURL}. Status: 404`,
      });
    });
  });
  ...
  describe('sourceLocationToRawLocation', () => {
    it('maps source locations to ranges correctly in the presence of inlining', async () => {
      ...
      const sources = await plugin.addRawModule(rawModuleId, '', {
        url: makeURL('/build/tests/inputs/inline.s.wasm'),
      });
      assert.deepEqual(sources, [sourceFileURL]);
      assert.deepEqual(
          await plugin.sourceLocationToRawLocation({
            rawModuleId,
            sourceFileURL,
            lineNumber: 0,
            columnNumber: -1,
  ...
  describe('getScopeInfo', () => {
    it('handles globals, locals, and parameters', async () => {
      ...
      assert.include(await plugin.getScopeInfo('GLOBAL'), {
        type: 'GLOBAL',
        typeName: 'Global',
      });
      assert.include(await plugin.getScopeInfo('LOCAL'), {
        type: 'LOCAL',
        typeName: 'Local',
      });
      assert.include(await plugin.getScopeInfo('PARAMETER'), {
        type: 'PARAMETER',
        typeName: 'Parameter',
      });
    });
  ...
  describe('getInlinedCalleeRanges', () => {
    it('gets inlined callee PC ranges correctly', async () => {
      ...
      const sources = await plugin.addRawModule('0', '', {
        url: makeURL('/build/tests/inputs/inline.s.wasm'),
      });
      assert.deepEqual(sources, [makeURL('/build/tests/inputs/inline.c')]);

      const ranges = await plugin.getInlinedCalleesRanges({
        rawModuleId: '0',
        codeOffset: 0x2,
        inlineFrameIndex: 0,
      });
      assert.deepEqual(ranges, [
        {rawModuleId: '0', startOffset: 0x5, endOffset: 0x6},
      ]);
    });
  });
  ...
  describe('getInlinedFunctionRanges', () => {
    it('gets inlined function PC ranges correctly', async () => {
      ...
      const ranges = await plugin.getInlinedFunctionRanges({
        rawModuleId: '0',
        codeOffset: 0x5,
        inlineFrameIndex: 0,
      });
  ...
  describe('getFunctionInfo', () => {
    it('gets inlined function infos correctly', async () => {
      ...
      const functions = await plugin.getFunctionInfo({
        rawModuleId: '0',
        codeOffset: 0x5,
        inlineFrameIndex: 0,
      });
      assert.deepEqual(functions, {
        frames: [{name: 'callee'}, {name: 'caller'}],
        missingSymbolFiles: [],
      });
    });
  });
  ...
  describe('listVariablesInScope', () => {
    it('lists parameter variables correctly', async () => {
      ...
      await plugin.addRawModule('0', '', {
        url: makeURL('/build/tests/inputs/shadowing.s.wasm'),
      });
    ...
    it('lists global variables correctly', async () => {
      ...
      await plugin.addRawModule('0', '', {
        url: makeURL('/build/tests/inputs/globals.s.wasm'),
      });
  ...
  describe('getMappedLines', () => {
    it('computes mapped lines correctly.', async () => {
      ...
      await plugin.addRawModule('hello', '', {
        url: makeURL('/build/tests/inputs/hello.s.wasm'),
      });

      const mappedLines = await plugin.getMappedLines(
          'hello',
          makeURL('/build/tests/inputs/hello.c'),
      );
    ...
    it('provides access to wasm state', async () => {
      class TestAsyncHostInterface implements AsyncHostInterface {
        readonly memory = {
          ...
          length: 5,
          stopId: 9,
          result: new Uint8Array([5, 6, 7, 8, 9]).buffer,
        };
        readonly local = {
          local: 9,
          stopId: 10,
          result: {type: 'i32', value: 5} as const,
        };
        readonly global = {
          global: 10,
          stopId: 11,
          result: {type: 'i32', value: 6} as const,
        };
        readonly op = {
          op: 11,
          stopId: 12,
          result: {type: 'i32', value: 7} as const,
        };
        async getWasmLinearMemory(
            offset: number,
            length: number,
            stopId: unknown,
            ): Promise<ArrayBuffer> {
        ...
        reportResourceLoad(
            _resourceUrl: string,
            _status: {success: boolean, errorMessage?: string, size?: number},
            ): Promise<void> {
      ...
      const worker = new Worker('/build/tests/DevToolsPluginTestWorker.js', {
        type: 'module',
      });
      const rpc = new WorkerRPC<AsyncHostInterface, TestWorkerInterface>(
          worker,
          hostInterface,
      );

      {
        ...
        const callResult = await rpc.sendMessage(
            'getWasmMemoryForTest',
            offset,
            length,
            stopId,
        );
