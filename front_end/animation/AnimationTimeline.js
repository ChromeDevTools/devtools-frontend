...
WebInspector.AnimationTimeline.prototype = {
  ...
  _addEventListeners: function(target) {
    ...
    animationModel.addEventListener(
        WebInspector.AnimationModel.Events.AnimationGroupStarted, this._animationGroupStarted,
        this);
    animationModel.addEventListener(
  ...
  _createScrubber: function() {
    ...
    this._timelineScrubberLine =
        this._timelineScrubber.createChild('div', 'animation-scrubber-line');
  ...
  _createHeader: function() {
    var toolbarContainer =
        this.contentElement.createChild('div', 'animation-timeline-toolbar-container');
    var topToolbar = new WebInspector.Toolbar('animation-timeline-toolbar', toolbarContainer);
    var clearButton =
        new WebInspector.ToolbarButton(WebInspector.UIString('Clear all'), 'clear-toolbar-item');
    clearButton.addEventListener('click', this._reset.bind(this));
    topToolbar.appendToolbarItem(clearButton);
    topToolbar.appendSeparator();

    this._pauseButton =
        new WebInspector.ToolbarToggle(WebInspector.UIString('Pause all'), 'pause-toolbar-item');
    ...
    var playbackRateControl =
        toolbarContainer.createChild('div', 'animation-playback-rate-control');
    ...
    for (var playbackRate of WebInspector.AnimationTimeline.GlobalPlaybackRates) {
      ...
      button.textContent = playbackRate ? WebInspector.UIString(playbackRate * 100 + '%') :
    ...
    this._popoverHelper = new WebInspector.PopoverHelper(
        this._previewContainer, this._getPopoverAnchor.bind(this), this._showPopover.bind(this),
    ...
    this._controlButton = new WebInspector.ToolbarButton(
        WebInspector.UIString('Replay timeline'), 'animation-control-toolbar-item');
    ...
    WebInspector.installDragHandle(
        ...
        this._scrubberDragMove.bind(this), this._scrubberDragEnd.bind(this), 'col-resize');
  ...
  _effectivePlaybackRate: function() {
    return (this._allPaused || (this._selectedGroup && this._selectedGroup.paused())) ?
        0 :
        this._playbackRate;
  },
  ...
  _animationGroupStarted: function(event) {
    this._addAnimationGroup(
        /** @type {!WebInspector.AnimationModel.AnimationGroup} */ (event.data));
  },
  ...
  _addAnimationGroup: function(group) {
     ...
     */
    function startTimeComparator(left, right) { return left.startTime() > right.startTime(); }
    ...
    while (this._groupBuffer.length > bufferSize) {
      var toDiscard =
          this._groupBuffer.splice(this._groupBuffer[0] === this._selectedGroup ? 1 : 0, 1);
  ...
  onResize: function() {
    this._cachedTimelineWidth =
        Math.max(0, this._animationsContainer.offsetWidth - this._timelineControlsWidth) || 0;
  ...
  _updateScrubber: function(timestamp) {
    if (!this._scrubberPlayer)
      return;
    this._currentTime.textContent =
        WebInspector.UIString(Number.millisToString(this._scrubberPlayer.currentTime));
  ...
  _scrubberDragMove: function(event) {
    ...
    var currentTime = Math.max(
        0, Math.min(this._originalScrubberTime + delta / this.pixelMsRatio(), this.duration()));
    this._scrubberPlayer.currentTime = currentTime;
    this._currentTime.textContent =
        WebInspector.UIString(Number.millisToString(Math.round(currentTime)));
...
WebInspector.AnimationTimeline.NodeUI.prototype = {
  ...
  _nodeChanged: function() {
    this.element.classList.toggle(
        ...
        this._node && this._node === WebInspector.context.flavor(WebInspector.DOMNode));
  }
};
