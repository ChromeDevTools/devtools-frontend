...
WebInspector.AnimationUI = function(animation, timeline, parentElement) {
  ...
  WebInspector.installDragHandle(
      ...
      this._mouseDown.bind(this, WebInspector.AnimationUI.MouseEvents.AnimationDrag, null),
      this._mouseMove.bind(this), this._mouseUp.bind(this), '-webkit-grabbing', '-webkit-grab');

  /** @type {!Array.<{group: ?Element, animationLine: ?Element, keyframePoints: !Object.<number,
   * !Element>, keyframeRender: !Object.<number, !Element>}>} */
...
WebInspector.AnimationUI.prototype = {
  /**
     ...
     */
  animation: function() { return this._animation; },

  /**
   * @param {?WebInspector.DOMNode} node
   */
  setNode: function(node) { this._node = node; },
  ...
  _drawAnimationLine: function(iteration, parentElement) {
    ...
    cache.animationLine.setAttribute(
        'x2', (this._duration() * this._timeline.pixelMsRatio() +
  ...
  _drawDelayLine: function(parentElement) {
    ...
    this._delayLine.setAttribute(
        'x2', (this._delay() * this._timeline.pixelMsRatio() + margin).toFixed(2));
    ...
    var leftMargin = Math.min(
        ...
        (this._delay() + this._duration() * this._animation.source().iterations()) *
    ...
    this._endDelayLine.setAttribute(
        'x2', forwardsFill ?
            ...
            (this._animation.source().endDelay() * this._timeline.pixelMsRatio() + margin)
                .toFixed(2));
  },
  ...
  _drawPoint: function(iteration, parentElement, x, keyframeIndex, attachEvents) {
    if (this._cachedElements[iteration].keyframePoints[keyframeIndex]) {
      this._cachedElements[iteration].keyframePoints[keyframeIndex].setAttribute(
          'cx', x.toFixed(2));
      return;
    }

    var circle = parentElement.createSVGChild(
  ...
  _renderKeyframe: function(iteration, keyframeIndex, parentElement, leftDistance, width, easing) {
    ...
    if (easing === 'linear') {
      ...
      group.setAttribute('d', [
        'M', 0, height, 'L', 0, 5, 'L', width.toFixed(2), 5, 'L', width.toFixed(2), height, 'Z'
      ].join(' '));
    } else if (bezier) {
      group.style.fill = this._color;
  ...
  redraw: function() {
    ...
    this._svg.setAttribute(
        'width', (maxWidth + 2 * WebInspector.AnimationUI.Options.AnimationMargin).toFixed(2));
    ...
    this._nameElement.style.transform = 'translateX(' +
        (this._delay() * this._timeline.pixelMsRatio() +
         WebInspector.AnimationUI.Options.AnimationMargin)
            .toFixed(2) +
        'px)';
    this._nameElement.style.width =
        (this._duration() * this._timeline.pixelMsRatio()).toFixed(2) + 'px';
    ...
    for (var iteration = 1; iteration < this._animation.source().iterations() &&
         iterationWidth * (iteration - 1) < this._timeline.width();
         iteration++)
  ...
  _renderTransition: function() {
    if (!this._cachedElements[0])
      this._cachedElements[0] =
          {animationLine: null, keyframePoints: {}, keyframeRender: {}, group: null};
    ...
    this._drawPoint(
        0, this._activeIntervalGroup, WebInspector.AnimationUI.Options.AnimationMargin, 0, true);
    this._drawPoint(
        0, this._activeIntervalGroup, this._duration() * this._timeline.pixelMsRatio() +
            WebInspector.AnimationUI.Options.AnimationMargin,
        -1, true);
  },
  ...
  _renderIteration: function(parentElement, iteration) {
    if (!this._cachedElements[iteration])
      this._cachedElements[iteration] = {
        ...
        keyframePoints: {},
        keyframeRender: {},
        group: parentElement.createSVGChild('g')
      };
    var group = this._cachedElements[iteration].group;
    group.style.transform = 'translateX(' +
        (iteration * this._duration() * this._timeline.pixelMsRatio()).toFixed(2) + 'px)';
    ...
    for (var i = 0; i < this._keyframes.length - 1; i++) {
      var leftDistance = this._offset(i) * this._duration() * this._timeline.pixelMsRatio() +
          WebInspector.AnimationUI.Options.AnimationMargin;
      var width = this._duration() * (this._offset(i + 1) - this._offset(i)) *
          this._timeline.pixelMsRatio();
    ...
    this._drawPoint(
        iteration, group, this._duration() * this._timeline.pixelMsRatio() +
            WebInspector.AnimationUI.Options.AnimationMargin,
        -1, iteration === 0);
  },
  ...
  _duration: function() {
    ...
    if (this._mouseEventType === WebInspector.AnimationUI.MouseEvents.FinishEndpointMove)
      ...
      duration -= Math.max(
          this._movementInMs, -this._animation.source().delay());  // Cannot have negative delay
  ...
  _offset: function(i) {
    ...
    if (this._mouseEventType === WebInspector.AnimationUI.MouseEvents.KeyframeMove &&
      ...
      console.assert(
          i > 0 && i < this._keyframes.length - 1, 'First and last keyframe cannot be moved');
...
WebInspector.AnimationUI.Color = function(animation) {
  ...
  var color =
      WebInspector.AnimationUI
          .Colors[names[String.hashCode(animation.name() || animation.id()) % names.length]];
