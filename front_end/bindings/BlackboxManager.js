...
WebInspector.BlackboxManager = function(debuggerWorkspaceBinding) {
  ...
  WebInspector.targetManager.addModelListener(
      WebInspector.DebuggerModel, WebInspector.DebuggerModel.Events.ParsedScriptSource,
...
WebInspector.BlackboxManager.prototype = {
  ...
  */
  ...
  sourceMapLoaded: function(script, sourceMap) {
    ...
    var positions = [];
    // If content in script file begin is not mapped and one or more ranges are blackboxed then
    // blackbox it.
  ...
  _uiSourceCodeURL: function(uiSourceCode) {
    return uiSourceCode.project().type() === WebInspector.projectTypes.Debugger ?
        null :
        uiSourceCode.url();
  },
  ...
  _unblackboxURL: function(url) {
    ...
    if (!regexValue)
      return;
    regexPatterns = regexPatterns.filter(function(item) { return item.pattern !== regexValue; });
    for (var i = 0; i < regexPatterns.length; ++i) {
      var item = regexPatterns[i];
      if (item.disabled)
        continue;
  ...
  _patternChanged: function() {
    ...
    function loadSourceMap(script) {
      return this.sourceMapLoaded(
          script, this._debuggerWorkspaceBinding.sourceMapForScript(script));
    }
  },
