...
WebInspector.DebuggerWorkspaceBinding = function(targetManager, workspace, networkMapping) {
  ...
  targetManager.addModelListener(
      ...
      this._globalObjectCleared, this);
  targetManager.addModelListener(
      WebInspector.DebuggerModel, WebInspector.DebuggerModel.Events.BeforeDebuggerPaused,
      this._beforeDebuggerPaused, this);
  targetManager.addModelListener(
...
WebInspector.DebuggerWorkspaceBinding.prototype = {
  ...
  targetAdded: function(target) {
    ...
    if (debuggerModel)
      this._targetToData.set(
          target, new WebInspector.DebuggerWorkspaceBinding.TargetData(debuggerModel, this));
  },
  ...
  uiLocationToRawLocations: function(uiSourceCode, lineNumber, columnNumber) {
    ...
    for (var i = 0; i < targetDatas.length; ++i) {
      var rawLocation =
          targetDatas[i]._uiLocationToRawLocation(uiSourceCode, lineNumber, columnNumber);
      if (rawLocation)
  ...
  normalizeUILocation: function(uiLocation) {
    ...
    if (target) {
      var rawLocation = this.uiLocationToRawLocation(
          target, uiLocation.uiSourceCode, uiLocation.lineNumber, uiLocation.columnNumber);
      if (rawLocation)
  ...
  _reset: function(target) {
    var targetData = this._targetToData.get(target);
    targetData.callFrameLocations.valuesArray().forEach(
        (location) => this._removeLiveLocation(location));
 ...
 */
WebInspector.DebuggerWorkspaceBinding.TargetData = function(
    debuggerModel, debuggerWorkspaceBinding) {
  ...
  this._defaultMapping =
      new WebInspector.DefaultScriptMapping(debuggerModel, workspace, debuggerWorkspaceBinding);
  this._resourceMapping = new WebInspector.ResourceScriptMapping(
      debuggerModel, workspace, networkMapping, debuggerWorkspaceBinding);
...
WebInspector.DebuggerWorkspaceBinding.TargetData.prototype = {
  ...
  _uiLocationToRawLocation: function(uiSourceCode, lineNumber, columnNumber) {
    ...
    return sourceMapping ?
        sourceMapping.uiLocationToRawLocation(uiSourceCode, lineNumber, columnNumber) :
        null;
  },
...
WebInspector.DebuggerWorkspaceBinding.ScriptInfo.prototype = {
   ...
   * @param {!WebInspector.LiveLocation} location
   */
  _removeLocation: function(location) { this._locations.delete(location); },
 ...
 * @param {!WebInspector.LiveLocationPool} locationPool
 */
WebInspector.DebuggerWorkspaceBinding.Location = function(
    script, rawLocation, binding, updateDelegate, locationPool) {
 ...
 * @param {!WebInspector.LiveLocationPool} locationPool
 */
WebInspector.DebuggerWorkspaceBinding.StackTraceTopFrameLocation = function(
    rawLocations, binding, updateDelegate, locationPool) {
  WebInspector.LiveLocationWithPool.call(this, updateDelegate, locationPool);
  ...
  for (var location of rawLocations)
    this._locations.add(
        binding.createLiveLocation(location, this._scheduleUpdate.bind(this), locationPool));
...
WebInspector.DebuggerWorkspaceBinding.StackTraceTopFrameLocation.prototype = {
  /**
     ...
     * @return {!WebInspector.UILocation}
     */
  uiLocation: function() { return this._current.uiLocation(); },

  /**
     * @override
     * @return {boolean}
     */
  isBlackboxed: function() { return this._current.isBlackboxed(); },
