...
(function(mod) {
  ...
  CodeMirror.commands.toggleComment = function(cm) { cm.toggleComment(); };
  ...
  CodeMirror.defineExtension('lineComment', function(from, to, options) {
    ...
    var end =
        Math.min(to.ch != 0 || to.line == from.line ? to.line + 1 : to.line, self.lastLine() + 1);
  ...
  CodeMirror.defineExtension('uncomment', function(from, to, options) {
    ...
    if (open == -1 || close == -1 || !/comment/.test(self.getTokenTypeAt(Pos(start, open + 1))) ||
        !/comment/.test(self.getTokenTypeAt(Pos(end, close + 1))))
      return false;

    // Avoid killing block comments completely outside the selection.
    // Positions of the last startString before the start of the selection, and the first endString
    // after it.
    var lastStart = startLine.lastIndexOf(startString, from.ch);
    var firstEnd = lastStart == -1 ?
        -1 :
        startLine.slice(0, from.ch).indexOf(endString, lastStart + startString.length);
    if (lastStart != -1 && firstEnd != -1 && firstEnd + endString.length != from.ch)
      return false;
    // Positions of the first endString after the end of the selection, and the last startString
    // before it.
    ...
    self.operation(function() {
      self.replaceRange(
          '', Pos(end, close -
                      (pad && endLine.slice(close - pad.length, close) == pad ? pad.length : 0)),
