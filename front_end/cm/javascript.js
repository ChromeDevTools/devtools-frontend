...
(function(mod) {
  ...
  CodeMirror.defineMode('javascript', function(config, parserConfig) {
    ...
    function pushlex(type, info) {
      var result = function() {
        ...
        if (state.lexical.type == 'stat')
          ...
          for (var outer = state.lexical; outer && outer.type == ')' && outer.align;
               outer = outer.prev)
    ...
    function statement(type, value) {
      ...
      if (type == 'switch')
        return cont(
            pushlex('form'), expression, pushlex('}', 'switch'), expect('{'), block, poplex,
            poplex);
      if (type == 'case')
      ...
      if (type == 'catch')
        return cont(
            pushlex('form'), pushcontext, expect('('), funarg, expect(')'), statement, poplex,
            popcontext);
      if (type == 'class')
      ...
      if (type == 'module')
        return cont(pushlex('form'), pattern, pushlex('}'), expect('{'), block, poplex, poplex) if (
            type ==
            'async') return cont(statement) return pass(pushlex('stat'), expression, expect(';'), poplex);
    }
    ...
    function expressionInner(type, noComma) {
      if (cx.state.fatArrowAt == cx.stream.start) {
        ...
        if (type == '(')
          return cont(
              pushcontext, pushlex(')'), commasep(pattern, ')'), poplex, expect('=>'), body,
              popcontext);
    ...
    function afterType(type, value) {
      if (value == '<')
        return cont(commasep(typeexpr, '>'), afterType) if (type == '[') return cont(
            expect(']'), afterType)
    }
    function vardef() { return pass(pattern, maybetype, maybeAssign, vardefCont); }
    function pattern(type, value) {
      if (type == 'modifier')
        return cont(pattern) if (type == 'variable') {
          register(value);
          return cont();
        }
      if (type == 'spread')
    ...
    function functiondef(type, value) {
      ...
      if (type == '(')
        return cont(
            pushcontext, pushlex(')'), commasep(funarg, ')'), poplex, maybetype, statement,
            popcontext);
    }
    ...
    return {
      ...
      skipExpression: function(state) {
        var top =
            state.cc[state.cc.length - 1] if (top == expression || top == expressionNoComma)
                state.cc.pop()
      }
    };
  });
