...
(function(mod) {
  ...
  CodeMirror.defineMode('clike', function(config, parserConfig) {
    var indentUnit = config.indentUnit,
        ...
        dontAlignCalls = parserConfig.dontAlignCalls, keywords = parserConfig.keywords || {},
        types = parserConfig.types || {}, builtin = parserConfig.builtin || {},
        blockKeywords = parserConfig.blockKeywords || {},
        defKeywords = parserConfig.defKeywords || {}, atoms = parserConfig.atoms || {},
        hooks = parserConfig.hooks || {}, multiLineStrings = parserConfig.multiLineStrings,
        indentStatements = parserConfig.indentStatements !== false,
        indentSwitch = parserConfig.indentSwitch !== false,
        namespaceSeparator = parserConfig.namespaceSeparator,
        isPunctuationChar = parserConfig.isPunctuationChar || /[\[\]{}\(\),;\:\.]/,
        numberStart = parserConfig.numberStart || /[\d\.]/, number = parserConfig.number ||
    ...
    function tokenBase(stream, state) {
      ...
      if (numberStart.test(ch)) {
        stream.backUp(1) if (stream.match(number)) return 'number'
        stream.next()
      }
      if (ch == '/') {
        if (stream.eat('*')) {
      ...
      token: function(stream, state) {
        ...
        if (endStatement.test(curPunc))
          ...
          popContext(state);
            ...
            indentStatements && (((ctx.type == '}' || ctx.type == 'top') && curPunc != ';') ||
  ...
  def(['text/x-csrc', 'text/x-c', 'text/x-chdr'], {
    ...
    types: words(
        cTypes + ' bool _Complex _Bool float_t double_t intptr_t intmax_t ' +
  ...
  def(['text/x-c++src', 'text/x-c++hdr'], {
    name: 'clike',
    keywords: words(
        cKeywords + ' asm dynamic_cast namespace reinterpret_cast try explicit new ' +
    ...
    hooks: {
      ...
      '=': function(stream, state) {
        ...
        if (cx.type == '}' && cx.align && stream.eat('>')) {
          state.context =
              new Context(cx.indented, cx.column, cx.type, cx.info, null, cx.prev) return 'operator'
        }
        else {
          return false
        }
  ...
  def('text/x-objectivec', {
    name: 'clike',
    keywords: words(
        cKeywords + 'inline restrict _Bool _Complex _Imaginary BOOL Class bycopy byref id IMP in ' +
