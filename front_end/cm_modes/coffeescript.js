...
(function(mod) {
  ...
  CodeMirror.defineMode('coffeescript', function(conf, parserConf) {
    ...
    var wordOperators =
        wordRegexp(['and', 'or', 'not', 'is', 'isnt', 'in', 'instanceof', 'typeof']);
    var indentKeywords = [
    ...
    function tokenBase(stream, state) {
      ...
      if (stream.match(regexPrefixes)) {
        if (stream.current() != '/' ||
            stream.match(/^.*\//, false)) {  // prevent highlight of division
    ...
    function indent(stream, state, type) {
      ...
      state.scope =
          {offset: offset, type: type, prev: state.scope, align: align, alignOffset: alignOffset};
    }
