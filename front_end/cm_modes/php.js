...
(function(mod) {
  ...
  function phpString_(stream, state, closing, escapes) {
      ...
      if (stream.match('[', false)) {
        ...
        state.tokenize = matchSequence(
            [
              [['[', null]],
              [
                [/\d[\w\.]*/, 'number'], [/\$[a-zA-Z_][a-zA-Z0-9_]*/, 'variable-2'],
                [/[\w\$]+/, 'variable']
              ],
              [[']', null]]
            ],
    ...
    var escaped = false;
    // Normal string
    while (!stream.eol() && (escaped || escapes === false ||
                             (!stream.match('{$', false) &&
  ...
  CodeMirror.registerHelper(
      'hintWords', 'php', [phpKeywords, phpAtoms, phpBuiltin].join(' ').split(' '));
  ...
  CodeMirror.defineMode('php', function(config, parserConfig) {
    ...
    return {
      startState: function() {
        var html = CodeMirror.startState(htmlMode) var php =
            parserConfig.startOpen ? CodeMirror.startState(phpMode) : null
        return {
          html: html,
          php: php,
      ...
      copyState: function(state) {
        var html = state.html, htmlNew = CodeMirror.copyState(htmlMode, html), php = state.php,
