...
(function(mod) {
  ...
  CodeMirror.defineMode('stylus', function(config) {
    var indentUnit = config.indentUnit, tagKeywords = keySet(tagKeywords_),
        tagVariablesRegexp = /^(a|b|i|s|col|em)$/i, propertyKeywords = keySet(propertyKeywords_),
        nonStandardPropertyKeywords = keySet(nonStandardPropertyKeywords_),
        valueKeywords = keySet(valueKeywords_), colorKeywords = keySet(colorKeywords_),
        documentTypes = keySet(documentTypes_), documentTypesRegexp = wordRegexp(documentTypes_),
        mediaFeatures = keySet(mediaFeatures_), mediaTypes = keySet(mediaTypes_),
        fontProperties = keySet(fontProperties_),
        operatorsRegexp = /^\s*([.]{2,3}|&&|\|\||\*\*|[?!=:]?=|[-+*\/%<>]=?|\?:|\~)/,
        wordOperatorKeywordsRegexp = wordRegexp(wordOperatorKeywords_),
        blockKeywords = keySet(blockKeywords_),
        vendorPrefixesRegexp = new RegExp(/^\-(moz|ms|o|webkit)-/i),
        commonAtoms = keySet(commonAtoms_), firstWordMatch = '', states = {}, ch, style, type,
        override;
    ...
    function typeIsPseudo(type, stream) { return type == ':' && stream.match(/^[a-z-]+/, false); }

    function startOfLine(stream) {
      return stream.sol() ||
          stream.string.match(new RegExp('^\\s*' + escapeRegExp(stream.current())));
    }

    function endOfLine(stream) { return stream.eol() || stream.match(/^\s*$/, false); }
    ...
    states.block = function(type, stream, state) {
      if ((type == 'comment' && startOfLine(stream)) || (type == ',' && endOfLine(stream)) ||
          type == 'mixin') {
        ...
        if (override == 'tag') {
          ...
          if (tagVariablesRegexp.test(word)) {
            if ((startOfLine(stream) && stream.string.match(/=/)) ||
                (!startOfLine(stream) && !stream.string.match(/^(\s*\.|#|\&|\[|\/|>|\*)/) &&
    ...
    states.keyframes = function(type, stream, state) {
      if (stream.indentation() == '0' &&
          ((type == '}' && startOfLine(stream)) || type == ']' || type == 'hash' ||
    ...
    return {
      ...
      indent: function(state, textAfter, line) {
        ...
        if (cx.prev &&
          ...
          if (/@|\$|\d/.test(ch) || /^\{/.test(textAfter) || /^\s*\/(\/|\*)/.test(textAfter) ||
              /^\s*\/\*/.test(prevLineFirstWord) ||
              /^\s*[\w-\.\[\]\'\"]+\s*(\?|:|\+)?=/i.test(textAfter) ||
              /^(\+|-)?[a-z][\w-]*\(/i.test(textAfter) || /^return/.test(textAfter) ||
            ...
            if (/\,\s*$/.test(prevLineFirstWord)) {
              indent = prevLineIndent;
                ...
                /^\s+/.test(line) && (/(\.|#|:|\[|\*|&|>|~|\+|\/)/.test(prevLineFirstWord) ||
            ...
            if (wordIsBlock(prevLineFirstWord)) {
              ...
              indent = lineIndent >= prevLineIndent ? prevLineIndent : lineIndent;
                ...
                /=\s*$/.test(prevLineFirstWord) || wordIsTag(prevLineFirstWord) ||
  ...
  var tagKeywords_ = [
    ...
    'b',        'base',     'bdi',      'bdo',      'bgsound',  'blockquote', 'body',
    ...
    'marquee',  'menu',     'menuitem', 'meta',     'meter',    'nav',        'nobr',
  ...
  var fontProperties_ = [
    'font-family', 'src', 'unicode-range', 'font-variant', 'font-feature-settings', 'font-stretch',
  ...
  var wordOperatorKeywords_ =
          ['in', 'and', 'or', 'not', 'is not', 'is a', 'is', 'isnt', 'defined', 'if unless'],
      blockKeywords_ = ['for', 'if', 'else', 'unless', 'from', 'to'],
      commonAtoms_ =
          ['null', 'true', 'false', 'href', 'title', 'type', 'not-allowed', 'readonly', 'disabled'],
      commonDef_ = [
        '@font-face', '@keyframes', '@media', '@viewport', '@page', '@host', '@supports', '@block',
        '@css'
      ];

  var hintWords = tagKeywords_.concat(
      documentTypes_, mediaTypes_, mediaFeatures_, propertyKeywords_, nonStandardPropertyKeywords_,
      colorKeywords_, valueKeywords_, fontProperties_, wordOperatorKeywords_, blockKeywords_,
