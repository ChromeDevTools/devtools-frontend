...
/** @typedef {{eventListeners:!Array<!WebInspector.EventListener>,
 * internalHandlers:?WebInspector.RemoteArray}} */
...
WebInspector.EventListener.frameworkEventListeners = function(object) {
  if (!object.target().hasDOMCapability()) {
    ...
    return Promise.resolve(/** @type {!WebInspector.FrameworkEventListenersObject} */ (
        {eventListeners: [], internalHandlers: null}));
  }

  var listenersResult =
      /** @type {!WebInspector.FrameworkEventListenersObject} */ ({eventListeners: []});
  ...
  /**
     ...
     */
  function getOwnProperties(object) { return object.getOwnPropertiesPromise(); }
  ...
  function convertToEventListeners(pageEventListenersObject) {
    ...
    function toEventListener(listenerObject) {
      ...
      var promises = [];
      promises.push(listenerObject.callFunctionJSONPromise(truncatePageEventListener, undefined)
      ...
      /**
             ...
             * @this {WebInspector.EventListenerObjectInInspectedPage}
             */
      function handlerFunction() { return this.handler; }
      ...
      function storeFunctionWithDetails(functionObject) {
        ...
        return /** @type {!Promise<undefined>} */ (functionObject.debuggerModel()
                                                       .functionDetailsPromise(functionObject)
                                                       .then(storeFunctionDetails));
      }
      ...
      /**
             ...
             * @this {WebInspector.EventListenerObjectInInspectedPage}
             */
      function getRemoveFunction() { return this.remove; }
   ...
   */
  function printErrorString(errorString) { WebInspector.console.error(errorString.value); }
  ...
  function filterOutEmptyObjects(objects) {
    ...
    /**
         ...
         * @template T
         */
    function filterOutEmpty(object) { return !!object; }
  }
      ...
      try {
        ...
        if (fetcherResult.eventListeners && isArrayLike(fetcherResult.eventListeners)) {
          eventListeners = eventListeners.concat(
              fetcherResult.eventListeners.map(checkEventListener).filter(nonEmptyObject));
        }
        if (fetcherResult.internalHandlers && isArrayLike(fetcherResult.internalHandlers))
          internalHandlers = internalHandlers.concat(
              fetcherResult.internalHandlers.map(checkInternalHandler).filter(nonEmptyObject));
      } catch (e) {
        ...
        for (var type in entryEvents) {
          ...
          for (var key in events) {
            if (typeof events[key] === 'function') {
              ...
              // We don't support removing for old version < 1.4 of jQuery because it doesn't
              // provide API for getting "selector".
              eventListeners.push(listener);
            }
          }
        }
