...
WebInspector.ExecutionContextSelector = function(targetManager, context) {
  ...
  context.addFlavorChangeListener(
...
WebInspector.ExecutionContextSelector.prototype = {
  ...
  _contextPersistentId: function(executionContext) {
    return executionContext.isDefault ?
        executionContext.target().name() + ':' + executionContext.frameId :
        '';
  },
  ...
  _shouldSwitchToContext: function(executionContext) {
    if (this._lastSelectedContextId &&
  ...
  _switchContextIfNecessary: function(executionContext) {
    if (!this._context.flavor(WebInspector.ExecutionContext) ||
        this._shouldSwitchToContext(executionContext)) {
      this._ignoreContextChanged = true;
...
WebInspector.ExecutionContextSelector.completionsForTextPromptInCurrentContext = function(
  ...
  WebInspector.ExecutionContextSelector.completionsForTextInCurrentContext(
      expressionRange.toString(), wordRange.toString(), force, completionsReadyCallback);
};
/**
 * @param {string} text
 ...
 * @param {function(!Array.<string>, number=)} completionsReadyCallback
 */
WebInspector.ExecutionContextSelector.completionsForTextInCurrentContext = function(
    text, completionsPrefix, force, completionsReadyCallback) {
  ...
  executionContext.completionsForExpression(
      clippedExpression, completionsPrefix, force, completionsReadyCallback);
};
