...
WebInspector.NetworkConditionsSelector = function(populateCallback, selectCallback) {
  ...
  this._manager.addEventListener(
      WebInspector.MultitargetNetworkManager.Events.ConditionsChanged, this._conditionsChanged,
      this);
...
WebInspector.NetworkConditionsSelector._conditionsTitle = function(conditions, plainText) {
  ...
  var downloadText =
      WebInspector.NetworkConditionsSelector._throughputText(conditions.download, plainText);
  var uploadText =
      WebInspector.NetworkConditionsSelector._throughputText(conditions.upload, plainText);
  var pattern = plainText ? '%s (%dms, %s, %s)' : '%s (%dms RTT, %s\u2b07, %s\u2b06)';
  var title =
      WebInspector.UIString(pattern, conditionTitle, conditions.latency, downloadText, uploadText);
  return {
    text: title,
...
WebInspector.NetworkConditionsSelector.prototype = {
  _populateOptions: function() {
    ...
    if (!this._conditionsChanged()) {
      for (var i = this._options.length - 1; i >= 0; i--) {
        if (this._options[i]) {
          this.optionSelected(
              /** @type {!WebInspector.NetworkManager.Conditions} */ (this._options[i]));
          break;
        }
      }
    }
   ...
   * @param {!WebInspector.NetworkManager.Conditions} conditions
   */
  optionSelected: function(conditions) { this._manager.setNetworkConditions(conditions); },
    ...
    for (var i = 0; i < groups.length; ++i) {
      ...
      if (!i) {
        groupElement.appendChild(
            new Option(WebInspector.UIString('Add\u2026'), WebInspector.UIString('Add\u2026')));
        options.push(null);
      }
...
WebInspector.NetworkConditionsSelector.createToolbarMenuButton = function() {
  ...
  function appendItems(contextMenu) {
    ...
    contextMenu.appendItem(
        WebInspector.UIString('Edit\u2026'), selector.revealAndUpdate.bind(selector));
  }
...
WebInspector.NetworkConditionsSelector.createOfflineToolbarCheckbox = function() {
  var checkbox = new WebInspector.ToolbarCheckbox(
      WebInspector.UIString('Offline'), WebInspector.UIString('Force disconnected from network'),
      undefined, forceOffline);
...
WebInspector.NetworkConditionsSettingsTab = function() {
  ...
  var addButton = createTextButton(
      WebInspector.UIString('Add custom profile...'), this._addButtonClicked.bind(this),
...
WebInspector.NetworkConditionsSettingsTab.prototype = {
  ...
  _addButtonClicked: function() {
    this._list.addNewItem(
        this._customSetting.get().length, {title: '', download: -1, upload: -1, latency: 0});
  },
  ...
  beginEdit: function(item) {
    ...
    editor.control('download').value =
        conditions.download <= 0 ? '' : String(conditions.download / (1024 / 8));
  ...
  _createEditor: function() {
    ...
    cell.appendChild(
        editor.createInput('download', 'text', WebInspector.UIString('kb/s'), throughputValidator));
    ...
    cell.appendChild(
        editor.createInput('upload', 'text', WebInspector.UIString('kb/s'), throughputValidator));
    cell.createChild('div', 'conditions-edit-optional').textContent =
        WebInspector.UIString('optional');
    fields.createChild('div', 'conditions-list-separator conditions-list-separator-invisible');
    ...
    cell = fields.createChild('div', 'conditions-list-text');
    cell.appendChild(
        editor.createInput('latency', 'text', WebInspector.UIString('ms'), latencyValidator));
    cell.createChild('div', 'conditions-edit-optional').textContent =
        WebInspector.UIString('optional');
