...
WebInspector.FilmStripModel.prototype = {
  ...
  reset: function(tracingModel, zeroTime) {
    ...
    for (var i = 0; i < events.length; ++i) {
      ...
      if (event.name === WebInspector.FilmStripModel.TraceEvents.CaptureFrame) {
        ...
        if (data)
          this._frames.push(
              WebInspector.FilmStripModel.Frame._fromEvent(this, event, this._frames.length));
        ...
        this._frames.push(WebInspector.FilmStripModel.Frame._fromSnapshot(
            this, /** @type {!WebInspector.TracingModel.ObjectSnapshot} */ (event),
            this._frames.length));
      }
    }
  },
  ...
  /**
     * @return {number}
     */
  zeroTime: function() { return this._zeroTime; },

  /**
     * @return {number}
     */
  spanTime: function() { return this._spanTime; },
  ...
  frameByTimestamp: function(timestamp) {
    var index =
        this._frames.upperBound(timestamp, (timestamp, frame) => timestamp - frame.timestamp) - 1;
