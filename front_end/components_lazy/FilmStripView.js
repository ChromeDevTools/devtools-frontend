...
WebInspector.FilmStripView.prototype = {
  ...
  setMode: function(mode) {
    ...
    this.contentElement.classList.toggle(
        'time-based', mode === WebInspector.FilmStripView.Modes.TimeBased);
    this.update();
  },
  ...
  createFrameElement: function(frame) {
    ...
    element.title =
        WebInspector.UIString('Doubleclick to zoom image. Click to view preceding requests.');
    ...
    element.addEventListener(
        'mousedown',
        this._onMouseEvent.bind(this, WebInspector.FilmStripView.Events.FrameSelected, time),
        false);
    element.addEventListener(
        'mouseenter',
        this._onMouseEvent.bind(this, WebInspector.FilmStripView.Events.FrameEnter, time), false);
    element.addEventListener(
        'mouseout',
        this._onMouseEvent.bind(this, WebInspector.FilmStripView.Events.FrameExit, time), false);
    ...
    /**
         * @return {!Element}
         */
    function returnElement() { return element; }
  },
  ...
  frameByTime: function(time) {
    /**
         ...
         * @return {number}
         */
    function comparator(time, frame) { return time - frame.timestamp; }
  ...
  update: function() {
    ...
    this.createFrameElement(frames[0]).then(continueWhenFrameImageLoaded.bind(
        this));  // Calculate frame width basing on the first frame.
    ...
    function continueWhenFrameImageLoaded(element0) {
      var frameWidth =
          Math.ceil(WebInspector.measurePreferredSize(element0, this.contentElement).width);
      if (!frameWidth)
        return;
   ...
   * @param {string} text
   */
  setStatusText: function(text) { this._statusLabel.textContent = text; },
...
WebInspector.FilmStripView.Dialog = function(filmStripFrame, zeroTime) {
  ...
  var prevButton = createTextButton(
      '\u25C0', this._onPrevFrame.bind(this), undefined, WebInspector.UIString('Previous frame'));
  footerElement.appendChild(prevButton);
  this._timeLabel = footerElement.createChild('div', 'filmstrip-dialog-label');
  var nextButton = createTextButton(
      '\u25B6', this._onNextFrame.bind(this), undefined, WebInspector.UIString('Next frame'));
