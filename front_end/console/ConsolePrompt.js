...
WebInspector.ConsolePrompt = function() {
  ...
  function gotFactory(factory) {
    this._editor = factory.createEditor(
        {lineNumbers: false, lineWrapping: true, mimeType: 'javascript', autoHeight: true});
...
WebInspector.ConsolePrompt.prototype = {
  ...
  /**
     * @return {string}
     */
  text: function() { return this._editor ? this._editor.text() : this._initialText; },

  /**
   * @param {boolean} value
   */
  setAddCompletionsFromHistory: function(value) { this._addCompletionsFromHistory = value; },
  ...
  _editorKeyDown: function(event) {
    ...
    switch (keyboardEvent.keyCode) {
      ...
      case WebInspector.KeyboardShortcut.Keys.P.code:  // Ctrl+P = Previous
        if (WebInspector.isMac() && keyboardEvent.ctrlKey && !keyboardEvent.metaKey &&
  ...
  _enterKeyPressed: function(event) {
    ...
    function compileCallback(scriptId, exceptionDetails) {
      if (str !== this.text())
        return;
      if (exceptionDetails &&
          (exceptionDetails.exception.description === 'SyntaxError: Unexpected end of input' ||
  ...
  _wordsWithPrefix: function(prefixRange, substituteRange) {
    ...
    var before = this._editor.text(
        new WebInspector.TextRange(0, 0, prefixRange.startLine, prefixRange.startColumn));
