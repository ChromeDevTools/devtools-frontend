...
describe('updateVersionFrom38To39', () => {
  ...
  it('sets the freestyler property to true if the closeable-tabs setting exists while keeping other properties intact',
     () => {
       const versionController = new VersionController();
       const closeableTabSetting = settings.createSetting('closeable-tabs', {someExistingProperty: true}) as
           Common.Settings.Setting<Record<string, boolean>>;

       versionController.updateVersionFrom38To39();
       assert.deepEqual(closeableTabSetting.get(), {someExistingProperty: true, freestyler: true});
     });

  it('creates the closeable-tabs setting with freestyler set to true if it does not exist', () => {
    const versionController = new VersionController();
    const closeableTabSetting = settings.createSetting('closeable-tabs', {});

    versionController.updateVersionFrom38To39();
    assert.deepEqual(closeableTabSetting.get(), {freestyler: true});
  });
});
