...
WebInspector.DevicesView = function() {
  ...
  this._discoveryListItem.addEventListener(
      'click',
      this._selectSidebarListItem.bind(this, this._discoveryListItem, this._discoveryView));
...
WebInspector.DevicesView.prototype = {
  ...
  _devicesDiscoveryConfigChanged: function(event) {
    ...
    var portForwardingConfig =
        /** @type {!Adb.PortForwardingConfig} */ (event.data['portForwardingConfig']);
...
WebInspector.DevicesView.DiscoveryView = function() {
  ...
  this.element.appendChild(createTextButton(
      WebInspector.UIString('Add rule'), this._addRuleButtonClicked.bind(this), 'add-rule-button'));
...
WebInspector.DevicesView.DiscoveryView.prototype = {
  ...
  discoveryConfigChanged: function(
    ...
    for (var key of Object.keys(portForwardingConfig)) {
      var rule =
          /** @type {!Adb.PortForwardingRule} */ ({port: key, address: portForwardingConfig[key]});
  ...
  _createEditor: function() {
    ...
    fields.createChild('div', 'port-forwarding-value port-forwarding-port')
        .appendChild(
            editor.createInput('port', 'text', 'Device port (3333)', portValidator.bind(this)));
    ...
    fields.createChild('div', 'port-forwarding-value')
        .appendChild(editor.createInput(
            'address', 'text', 'Local address (dev.example.corp:3333)', addressValidator));
    return editor;
  ...
  _updateDiscoveryConfig: function() {
    ...
    InspectorFrontendHost.setDevicesDiscoveryConfig(
        this._discoverUsbDevicesCheckbox.checked, this._portForwardingEnabledCheckbox.checked,
        configMap);
  },
...
/** @typedef {!{browser: ?Adb.Browser, element: !Element, title: !Element, pages: !Element,
 * viewMore: !Element, newTab: !Element, pageSections: !Map<string,
 * !WebInspector.DevicesView.PageSection>}} */
WebInspector.DevicesView.BrowserSection;

/** @typedef {!{page: ?Adb.Page, element: !Element, title: !Element, url: !Element, inspect:
 * !Element}} */
WebInspector.DevicesView.PageSection;
...
WebInspector.DevicesView.DeviceView.prototype = {
    ...
    if (!section.browser || section.browser.adbBrowserName !== browser.adbBrowserName ||
      ...
      if (browser.adbBrowserVersion)
        section.title.textContent =
            String.sprintf('%s (%s)', browser.adbBrowserName, browser.adbBrowserVersion);
      else
  ...
  _createPageSection: function() {
    ...
    var inspect = createTextButton(
        WebInspector.UIString('Inspect'), doAction.bind(null, 'inspect'), 'device-inspect-button');
