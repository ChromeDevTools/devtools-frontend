...
WebInspector.ClassesPaneWidget._classesSymbol =
    Symbol('WebInspector.ClassesPaneWidget._classesSymbol');
...
WebInspector.ClassesPaneWidget.prototype = {
  ...
  _installNodeClasses: function(node) {
     ...
     */
    function onClassNameUpdated() { this._mutatingNodes.delete(node); }
  },
  ...
  _getClassNames: function(selectedNode) {
    ...
    for (var stylesheet of allStyleSheets) {
      if (stylesheet.frameId !== this._selectedFrameId)
        continue;
      var cssPromise =
          cssModel.classNamesPromise(stylesheet.id).then(classes => completions.addAll(classes));
