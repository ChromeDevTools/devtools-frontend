...
WebInspector.BezierPopoverIcon.prototype = {
  ...
  _iconClick: function(event) {
    ...
    this._bezierEditor.addEventListener(
        WebInspector.BezierEditor.Events.BezierChanged, this._boundBezierChanged);
    this._swatchPopoverHelper.show(
        this._bezierEditor, this._swatch.iconElement(), this._onPopoverHidden.bind(this));
    ...
    var uiLocation = WebInspector.cssWorkspaceBinding.propertyUILocation(
   ...
   * @param {!Event} event
   */
  _onScroll: function(event) { this._swatchPopoverHelper.reposition(); },
  ...
  _onPopoverHidden: function(commitEdit) {
    ...
    this._bezierEditor.removeEventListener(
        WebInspector.BezierEditor.Events.BezierChanged, this._boundBezierChanged);
    delete this._bezierEditor;

    var propertyText =
        commitEdit ? this._treeElement.renderedPropertyText() : this._originalPropertyText;
...
WebInspector.ColorSwatchPopoverIcon = function(treeElement, swatchPopoverHelper, swatch) {
  ...
  this._swatch.iconElement().title =
      WebInspector.UIString('Open color picker. %s', shiftClickMessage);
...
WebInspector.ColorSwatchPopoverIcon.prototype = {
  ...
  showPopover: function() {
    ...
    this._spectrum.addEventListener(
        WebInspector.Spectrum.Events.ColorChanged, this._boundSpectrumChanged);
    this._swatchPopoverHelper.show(
        this._spectrum, this._swatch.iconElement(), this._onPopoverHidden.bind(this));
    this._scrollerElement = this._swatch.enclosingNodeOrSelfWithClass('style-panes-wrapper');
    if (this._scrollerElement)
      this._scrollerElement.addEventListener('scroll', this._boundOnScroll, false);
  ...
  _onPopoverHidden: function(commitEdit) {
    ...
    this._spectrum.removeEventListener(
        WebInspector.Spectrum.Events.ColorChanged, this._boundSpectrumChanged);
...
WebInspector.ShadowSwatchPopoverHelper.prototype = {
  ...
  showPopover: function() {
    ...
    this._cssShadowEditor.addEventListener(
        WebInspector.CSSShadowEditor.Events.ShadowChanged, this._boundShadowChanged);
    this._swatchPopoverHelper.show(
        this._cssShadowEditor, this._iconElement, this._onPopoverHidden.bind(this));
