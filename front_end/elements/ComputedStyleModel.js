...
WebInspector.ComputedStyleModel.prototype = {
  ...
  _updateTarget: function(target) {
    ...
    if (this._cssModel && domModel && resourceTreeModel) {
      this._targetEvents = [
        ...
        this._cssModel.addEventListener(
            WebInspector.CSSModel.Events.FontsUpdated, this._onComputedStyleChanged, this),
        this._cssModel.addEventListener(
            WebInspector.CSSModel.Events.MediaQueryResultChanged, this._onComputedStyleChanged,
            this),
        this._cssModel.addEventListener(
        ...
        domModel.addEventListener(
            WebInspector.DOMModel.Events.DOMMutated, this._onDOMModelChanged, this),
  ...
  */
  _onDOMModelChanged: function(event) {
    ...
    if (!this._node ||
        this._node !== node && node.parentNode !== this._node.parentNode &&
            !node.isAncestor(this._node))
      return;
    this._onComputedStyleChanged(null);
  },
  ...
  /**
     * @return {?WebInspector.DOMNode}
     */
  _elementNode: function() { return this.node() ? this.node().enclosingElementOrSelf() : null; },
  ...
  fetchComputedStyle: function() {
    ...
    if (!this._computedStylePromise)
      this._computedStylePromise = cssModel.computedStylePromise(elementNode.id)
                                       .then(verifyOutdated.bind(this, elementNode));
