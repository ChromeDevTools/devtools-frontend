...
WebInspector.ElementStatePaneWidget = function() {
  ...
  function clickListener(event) {
    ...
    if (!node)
      return;
    WebInspector.CSSModel.fromNode(node).forcePseudoState(
        node, event.target.state, event.target.checked);
  }
  ...
  _updateTarget: function(target) {
    ...
    if (this._target) {
      ...
      cssModel.removeEventListener(
          WebInspector.CSSModel.Events.PseudoStateForced, this._update, this);
    }
    this._target = target;
    if (target) {
...
WebInspector.ElementStatePaneWidget.ButtonProvider.prototype = {
  _clicked: function() {
    WebInspector.ElementsPanel.instance().showToolbarPane(
        !this._view.isShowing() ? this._view : null, this._button);
  },
