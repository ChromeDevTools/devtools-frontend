...
WebInspector.AdvancedApp.prototype = {
  ...
  presentUI: function(document) {
    ...
    this._rootSplitWidget =
        new WebInspector.SplitWidget(false, true, 'InspectorView.splitViewState', 555, 300, true);
    ...
    this._inspectedPagePlaceholder.addEventListener(
        ...
        this._onSetInspectedPageBounds.bind(this), this);
    this._deviceModeView = new WebInspector.DeviceModeWrapper(this._inspectedPagePlaceholder);

    WebInspector.dockController.addEventListener(
        WebInspector.DockController.Events.BeforeDockSideChanged, this._onBeforeDockSideChange,
        this);
    WebInspector.dockController.addEventListener(
  ...
  toolboxLoaded: function(toolboxDocument) {
    WebInspector.initializeUIUtils(
        toolboxDocument, WebInspector.settings.createSetting('uiTheme', 'default'));
  ...
  _onDockSideChange: function(event) {
    ...
    var toDockSide =
        event ? /** @type {string} */ (event.data.to) : WebInspector.dockController.dockSide();
