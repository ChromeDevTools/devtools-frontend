...
WebInspector.DeviceModeModel = function(updateCallback) {
  ...
  if (this._heightSetting.get() &&
      this._heightSetting.get() < WebInspector.DeviceModeModel.MinDeviceSize)
  ...
  this._uaSetting = WebInspector.settings.createSetting(
      'emulation.deviceUA', WebInspector.DeviceModeModel.UA.Mobile);
  this._uaSetting.addChangeListener(this._uaSettingChanged, this);
  this._deviceScaleFactorSetting =
      WebInspector.settings.createSetting('emulation.deviceScaleFactor', 0);
...
WebInspector.DeviceModeModel.prototype = {
  ...
  emulate: function(type, device, mode) {
    var resetPageScaleFactor =
        this._type !== type || this._device !== device || this._mode !== mode;
    this._type = type;
    ...
    if (type === WebInspector.DeviceModeModel.Type.Device) {
      ...
      if (this._initialized) {
        ...
        this._scaleSetting.set(this._calculateFitScale(
   ...
   * @param {number} scale
   */
  setScale: function(scale) { this._scaleSetting.set(scale); },

  /**
     * @return {?WebInspector.EmulatedDevice}
     */
  device: function() { return this._device; },
  ...
  outlineImage: function() {
    return (this._device && this._mode && this._deviceOutlineSetting.get()) ?
  ...
  /**
     ...
     */
  outlineRect: function() { return this._outlineRect; },

  /**
     * @return {!WebInspector.Rect}
     */
  screenRect: function() { return this._screenRect; },

  /**
     * @return {!WebInspector.Rect}
     */
  visiblePageRect: function() { return this._visiblePageRect; },

  /**
     * @return {number}
     */
  scale: function() { return this._scale; },

  /**
     * @return {number}
     */
  fitScale: function() { return this._fitScale; },

  /**
     * @return {!Size}
     */
  appliedDeviceSize: function() { return this._appliedDeviceSize; },

  /**
     * @return {number}
     */
  appliedDeviceScaleFactor: function() { return this._appliedDeviceScaleFactor; },

  /**
     * @return {!WebInspector.DeviceModeModel.UA}
     */
  appliedUserAgentType: function() { return this._appliedUserAgentType; },

  /**
     * @return {boolean}
     */
  isFullHeight: function() { return !this._heightSetting.get(); },

  /**
     * @return {!WebInspector.Setting}
     */
  scaleSetting: function() { return this._scaleSetting; },

  /**
     * @return {!WebInspector.Setting}
     */
  uaSetting: function() { return this._uaSetting; },

  /**
     * @return {!WebInspector.Setting}
     */
  deviceScaleFactorSetting: function() { return this._deviceScaleFactorSetting; },

  /**
     * @return {!WebInspector.Setting}
     */
  deviceOutlineSetting: function() { return this._deviceOutlineSetting; },
  ...
  _scaleSettingChanged: function() { this._calculateAndEmulate(false); },

  _widthSettingChanged: function() { this._calculateAndEmulate(false); },

  _heightSettingChanged: function() { this._calculateAndEmulate(false); },

  _uaSettingChanged: function() { this._calculateAndEmulate(true); },

  _deviceScaleFactorSettingChanged: function() { this._calculateAndEmulate(false); },

  _deviceOutlineSettingChanged: function() { this._calculateAndEmulate(false); },
  ...
  _calculateAndEmulate: function(resetPageScaleFactor) {
    ...
    if (this._type === WebInspector.DeviceModeModel.Type.Device) {
      ...
      this._fitScale =
          this._calculateFitScale(orientation.width, orientation.height, outline, insets);
      if (this._device.mobile())
        this._appliedUserAgentType = this._device.touch() ?
      ...
      this._applyDeviceMetrics(
          new Size(orientation.width, orientation.height), insets, outline,
          this._scaleSetting.get(), this._device.deviceScaleFactor, this._device.mobile(),
          this._mode.orientation === WebInspector.EmulatedDevice.Horizontal ? 'landscapePrimary' :
                                                                              'portraitPrimary',
      ...
      this._appliedUserAgentType = WebInspector.DeviceModeModel.UA.Desktop;
      this._applyDeviceMetrics(
          this._availableSize, new Insets(0, 0, 0, 0), new Insets(0, 0, 0, 0), 1, 0, false, '',
          resetPageScaleFactor);
      ...
      var defaultDeviceScaleFactor =
          mobile ? WebInspector.DeviceModeModel.defaultMobileScaleFactor : 0;
      ...
      this._applyDeviceMetrics(
          new Size(screenWidth, screenHeight), new Insets(0, 0, 0, 0), new Insets(0, 0, 0, 0),
          this._scaleSetting.get(),
          this._deviceScaleFactorSetting.get() || defaultDeviceScaleFactor, mobile,
          screenHeight >= screenWidth ? 'portraitPrimary' : 'landscapePrimary',
          resetPageScaleFactor);
    ...
    if (this._target)
      this._target.renderingAgent().setShowViewportSizeOnResize(
          this._type === WebInspector.DeviceModeModel.Type.None);
   ...
   * @param {boolean} resetPageScaleFactor
   */
  _applyDeviceMetrics: function(
      screenSize, insets, outline, scale, deviceScaleFactor, mobile, screenOrientation,
      resetPageScaleFactor) {
    ...
    this._screenRect = new WebInspector.Rect(
        ...
        outline.top * scale, screenSize.width * scale, screenSize.height * scale);
    this._outlineRect = new WebInspector.Rect(
        this._screenRect.left - outline.left * scale, 0,
    ...
    this._visiblePageRect = new WebInspector.Rect(
        positionX * scale, positionY * scale, Math.min(
                                                  pageWidth * scale, this._availableSize.width -
                                                      this._screenRect.left - positionX * scale),
        Math.min(
            pageHeight * scale,
            this._availableSize.height - this._screenRect.top - positionY * scale));
    this._scale = scale;

    if (scale === 1 && this._availableSize.width >= screenSize.width &&
        this._availableSize.height >= screenSize.height) {
      // When we have enough space, no page size override is required. This will speed things up and
      // remove lag.
      pageWidth = 0;
      pageHeight = 0;
    }
    if (this._visiblePageRect.width === pageWidth * scale &&
        this._visiblePageRect.height === pageHeight * scale &&
        Number.isInteger(pageWidth * scale) && Number.isInteger(pageHeight * scale)) {
      // When we only have to apply scale, do not resize the page. This will speed things up and
      // remove lag.
      pageWidth = 0;
      pageHeight = 0;
    }
