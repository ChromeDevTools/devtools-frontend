...
WebInspector.DevicesSettingsTab = function() {
  ...
  this._addCustomButton = createTextButton(
      WebInspector.UIString('Add custom device...'), this._addCustomDevice.bind(this));
...
WebInspector.DevicesSettingsTab.prototype = {
  ...
  commitEdit: function(item, editor, isNew) {
    ...
    device.vertical.width =
        editor.control('width').value ? parseInt(editor.control('width').value, 10) : 0;
    device.vertical.height =
        editor.control('height').value ? parseInt(editor.control('height').value, 10) : 0;
    device.horizontal.width = device.vertical.height;
    device.horizontal.height = device.vertical.width;
    device.deviceScaleFactor =
        editor.control('scale').value ? parseFloat(editor.control('scale').value) : 0;
  ...
  _createEditor: function() {
    ...
    fields.createChild('div', 'hbox')
        .appendChild(editor.createInput(
            'title', 'text', WebInspector.UIString('Device name'), titleValidator));
    var screen = fields.createChild('div', 'hbox');
    screen.appendChild(
        editor.createInput('width', 'text', WebInspector.UIString('Width'), sizeValidator));
    screen.appendChild(
        editor.createInput('height', 'text', WebInspector.UIString('height'), sizeValidator));
    var dpr = editor.createInput(
        'scale', 'text', WebInspector.UIString('Device pixel ratio'), scaleValidator);
    ...
    ua.appendChild(editor.createInput(
        'user-agent', 'text', WebInspector.UIString('User agent string'), () => true));
