...
WebInspector.EmulatedDevice = function() {
  ...
  /** @type {!Array.<string>} */
  this.capabilities =
      [WebInspector.EmulatedDevice.Capability.Touch, WebInspector.EmulatedDevice.Capability.Mobile];
  /** @type {string} */
  this.userAgent = '';
  ...
  try {
    ...
    function parseValue(object, key, type, defaultValue) {
      ...
      if (typeof value !== type || value === null)
        throw new Error(
            'Emulated device property \'' + key + '\' has wrong type \'' + typeof value + '\'');
      return value;
    }
    ...
    function parseOrientation(json) {
      ...
      if (outlineInsets) {
        ...
        result.outlineImage =
            /** @type {string} */ (parseValue(json['outline'], 'image', 'string'));
      }
    ...
    result.userAgent =
        WebInspector.MultitargetNetworkManager.patchUserAgentWithChromeVersion(rawUserAgent);
    ...
    result.deviceScaleFactor =
        /** @type {number} */ (parseValue(json['screen'], 'device-pixel-ratio', 'number'));
    ...
    for (var i = 0; i < modes.length; ++i) {
      ...
      if (mode.insets.top < 0 || mode.insets.left < 0 || mode.insets.right < 0 ||
          mode.insets.bottom < 0 || mode.insets.top + mode.insets.bottom > orientation.height ||
    ...
    result._showByDefault =
        /** @type {boolean} */ (parseValue(json, 'show-by-default', 'boolean', undefined));
    result._show = /** @type {string} */ (
        parseValue(json, 'show', 'string', WebInspector.EmulatedDevice._Show.Default));
...
WebInspector.EmulatedDevice.prototype = {
  /**
     ...
     */
  extension: function() { return this._extension; },
  ...
  setShow: function(show) {
    this._show =
        show ? WebInspector.EmulatedDevice._Show.Always : WebInspector.EmulatedDevice._Show.Never;
  },

  /**
   * @param {!WebInspector.EmulatedDevice} other
   */
  copyShowFrom: function(other) { this._show = other._show; },
...
WebInspector.EmulatedDevicesList.prototype = {
  ...
  /**
     ...
     */
  standard: function() { return this._standard; },

  /**
     * @return {!Array.<!WebInspector.EmulatedDevice>}
     */
  custom: function() { return this._custom; },

  revealCustomSetting: function() { WebInspector.Revealer.reveal(this._customSetting); },
  ...
  saveCustomDevices: function() {
    var json = this._custom.map(/** @param {!WebInspector.EmulatedDevice} device */ function(
        device) { return device._toJSON(); });
  ...
  saveStandardDevices: function() {
    var json = this._standard.map(/** @param {!WebInspector.EmulatedDevice} device */ function(
        device) { return device._toJSON(); });
...
WebInspector.EmulatedDevicesList.instance = function() {
  ...
  return /** @type {!WebInspector.EmulatedDevicesList} */ (
      WebInspector.EmulatedDevicesList._instance);
};
