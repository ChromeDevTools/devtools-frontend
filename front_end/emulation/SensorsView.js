...
WebInspector.SensorsView = function() {
  ...
  this._geolocationSetting =
      WebInspector.settings.createSetting('emulation.geolocationOverride', '');
  ...
  this._deviceOrientationSetting =
      WebInspector.settings.createSetting('emulation.deviceOrientationOverride', '');
  this._deviceOrientation =
      WebInspector.DeviceOrientation.parseSetting(this._deviceOrientationSetting.get());
...
WebInspector.SensorsView.prototype = {
  ...
  _createGeolocationSection: function(geolocation) {
    ...
    geogroup.createChild('div', 'sensors-group-title').textContent =
        WebInspector.UIString('Geolocation');
    ...
    this._locationSelectElement.appendChild(
        new Option(noOverrideOption.title, noOverrideOption.location));
    this._locationSelectElement.appendChild(
        new Option(customLocationOption.title, customLocationOption.location));
    ...
    this._locationSelectElement.addEventListener(
        'change', this._geolocationSelectChanged.bind(this));
    ...
    this._latitudeSetter = WebInspector.bindInput(
        ...
        WebInspector.Geolocation.latitudeValidator, true);
    ...
    this._longitudeSetter = WebInspector.bindInput(
        ...
        WebInspector.Geolocation.longitudeValidator, true);
    this._longitudeSetter(String(geolocation.longitude));

    latitudeGroup.createChild('div', 'latlong-title').textContent =
        WebInspector.UIString('Latitude');
    longitudeGroup.createChild('div', 'latlong-title').textContent =
        WebInspector.UIString('Longitude');
  },

  _geolocationSelectChanged: function() {
    this._fieldsetElement.disabled = false;
    var value =
        this._locationSelectElement.options[this._locationSelectElement.selectedIndex].value;
  ...
  _applyGeolocationUserInput: function() {
    ...
    this._setSelectElementLabel(
        this._locationSelectElement, WebInspector.SensorsView.NonPresetOptions.Custom);
  ...
  _createDeviceOrientationSection: function() {
    ...
    orientationGroup.createChild('div', 'sensors-group-title').textContent =
    ...
    this._orientationSelectElement.addEventListener(
        'change', this._orientationSelectChanged.bind(this));

    this._deviceOrientationFieldset =
        this._createDeviceOrientationOverrideElement(this._deviceOrientation);

    this._stageElement = orientationContent.createChild('div', 'orientation-stage');
    this._stageElement.title =
        WebInspector.UIString('Shift+drag horizontally to rotate around the y-axis');
    this._orientationLayer = this._stageElement.createChild('div', 'orientation-layer');
    this._boxElement =
        this._orientationLayer.createChild('section', 'orientation-box orientation-element');
    ...
    WebInspector.installDragHandle(
        this._stageElement, this._onBoxDragStart.bind(this), this._onBoxDrag.bind(this), null,
   ...
   */
  _enableOrientationFields: function(disable) {
    if (disable) {
  ...
  _orientationSelectChanged: function() {
    var value =
        this._orientationSelectElement.options[this._orientationSelectElement.selectedIndex].value;
    ...
    if (value === WebInspector.SensorsView.NonPresetOptions.NoOverride) {
      ...
      this._deviceOrientationOverrideEnabled = true;
      this._deviceOrientation =
          new WebInspector.DeviceOrientation(parsedValue[0], parsedValue[1], parsedValue[2]);
      this._setDeviceOrientation(
          this._deviceOrientation,
          WebInspector.SensorsView.DeviceOrientationModificationSource.SelectPreset);
    }
  },
  ...
  _applyDeviceOrientationUserInput: function() {
    this._setDeviceOrientation(
        WebInspector.DeviceOrientation.parseUserInput(
            this._alphaElement.value.trim(), this._betaElement.value.trim(),
            this._gammaElement.value.trim()),
        WebInspector.SensorsView.DeviceOrientationModificationSource.UserInput);
    this._setSelectElementLabel(
        this._orientationSelectElement, WebInspector.SensorsView.NonPresetOptions.Custom);
  },
  ...
  _setDeviceOrientation: function(deviceOrientation, modificationSource) {
    ...
    var animate = modificationSource !==
        WebInspector.SensorsView.DeviceOrientationModificationSource.UserDrag;
  ...
  _createAxisInput: function(parentElement, input, label) {
    ...
    return WebInspector.bindInput(
        ...
        WebInspector.DeviceOrientation.validator, true);
  },
  ...
  _createDeviceOrientationOverrideElement: function(deviceOrientation) {
    ...
    this._alphaSetter = this._createAxisInput(
        cellElement, this._alphaElement, WebInspector.UIString('\u03B1 (alpha)'));
    ...
    this._betaSetter = this._createAxisInput(
        cellElement, this._betaElement, WebInspector.UIString('\u03B2 (beta)'));
    ...
    this._gammaSetter = this._createAxisInput(
        cellElement, this._gammaElement, WebInspector.UIString('\u03B3 (gamma)'));
    this._gammaSetter(String(deviceOrientation.gamma));

    cellElement.appendChild(createTextButton(
        WebInspector.UIString('Reset'), this._resetDeviceOrientation.bind(this),
  ...
  _setBoxOrientation: function(deviceOrientation, animate) {
    ...
    this._boxMatrix =
        matrix.rotate(-deviceOrientation.beta, deviceOrientation.gamma, -deviceOrientation.alpha);
    var eulerAngles = new WebInspector.Geometry.EulerAngles(
  ...
  _onBoxDrag: function(event) {
    ...
    if (event.shiftKey) {
      ...
      angle = (this._mouseDownVector.x - mouseMoveVector.x) *
          WebInspector.SensorsView.ShiftDragOrientationSpeed;
    } else {
    ...
    var newOrientation = new WebInspector.DeviceOrientation(
        -eulerAngles.alpha, -eulerAngles.beta, eulerAngles.gamma);
    this._setDeviceOrientation(
        newOrientation, WebInspector.SensorsView.DeviceOrientationModificationSource.UserDrag);
    this._setSelectElementLabel(
        this._orientationSelectElement, WebInspector.SensorsView.NonPresetOptions.Custom);
    return false;
  },
  ...
  _appendTouchControl: function() {
    ...
    function applyTouch() {
      WebInspector.MultitargetTouchModel.instance().setCustomTouchEnabled(
...
/** @type {!Array.<{title: string, value: !Array.<{title: string, orientation:
