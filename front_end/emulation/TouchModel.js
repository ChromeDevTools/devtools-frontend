...
WebInspector.MultitargetTouchModel.prototype = {
  ...
  _applyToTarget: function(target) {
    ...
    if (previous.enabled === current.enabled &&
        (!current.enabled || previous.configuration === current.configuration))
      return;
    ...
    if (current.enabled)
      target.pageAgent().addScriptToEvaluateOnLoad(
          '(' + injectedFunction.toString() + ')()', scriptAddedCallback);
...
WebInspector.MultitargetTouchModel.instance = function() {
  ...
  return /** @type {!WebInspector.MultitargetTouchModel} */ (
      WebInspector.MultitargetTouchModel._instance);
};
