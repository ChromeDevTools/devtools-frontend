...
WebInspector.FormattedContentBuilder.prototype = {
  ...
  addToken: function(token, offset) {
    ...
    if (this._enforceSpaceBetweenWords && last && /\w/.test(last[last.length - 1]) &&
        /\w/.test(token))
      this.addSoftSpace();
  ...
  increaseNestingLevel: function() { this._nestingLevel += 1; },
  ...
  /**
     * @return {string}
     */
  content: function() { return this._formattedContent.join('') + (this._newLines ? '\n' : ''); },

  /**
     * @return {!{original: !Array.<number>, formatted: !Array.<number>}}
     */
  mapping: function() { return this._mapping; },
  ...
  _addMappingIfNeeded: function(originalPosition) {
    if (originalPosition - this._lastOriginalPosition ===
        this._formattedContentLength - this._lastFormattedPosition)
      return;
