...
WebInspector.HTMLFormatter.prototype = {
   ...
   * @param {!WebInspector.HTMLModel.Element} element
   */
  _afterCloseTag: function(element) { this._builder.addNewLine(); },
  ...
  _formatToken: function(element, token) {
    ...
    var isBodyToken = element.openTag.endOffset <= token.startOffset &&
        token.startOffset < element.closeTag.startOffset;
    ...
    if (isBodyToken && element.name === 'script') {
      this._builder.addNewLine();
      this._builder.increaseNestingLevel();
      var mimeType = element.openTag.attributes.has('type') ?
          element.openTag.attributes.get('type').toLowerCase() :
          null;
...
WebInspector.HTMLModel.prototype = {
  ...
  _build: function(text) {
    ...
    while (true) {
      ...
      this._tokens.push(
          new WebInspector.HTMLModel.Token(tokenValue, new Set(), tokenStart, tokenEnd));
    }

    while (this._stack.length > 1) {
      var element = this._stack.peekLast();
      this._popElement(new WebInspector.HTMLModel.Tag(
          element.name, text.length, text.length, new Map(), false, false));
    }
  ...
  _onEndTag: function(token) {
    ...
    var selfClosingTag =
        token.value === '/>' || WebInspector.HTMLModel.SelfClosingTags.has(this._tagName);
    var tag = new WebInspector.HTMLModel.Tag(
        this._tagName, this._tagStartOffset, this._tagEndOffset, this._attributes, this._isOpenTag,
        selfClosingTag);
  ...
  /**
     * @return {?WebInspector.HTMLModel.Token}
     */
  nextToken: function() { return this._tokens[this._tokenIndex++]; },
 ...
 */
WebInspector.HTMLModel.Tag = function(
    name, startOffset, endOffset, attributes, isOpenTag, selfClosingTag) {
  this.name = name;
  this.startOffset = startOffset;
  this.endOffset = endOffset;
