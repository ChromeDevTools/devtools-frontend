...
WebInspector.SCSSParser = function() {}
  ...
  _handleInclude: function(node, output) {
    ...
    if (!mixinArguments)
      return;
    var parameters =
        mixinArguments.content.filter(node => node.type !== 'delimiter' && node.type !== 'space');
 ...
 */
  ...
  return new WebInspector.TextRange(
      node.start.line - 1, node.start.column - 1, node.end.line - 1, node.end.column);
}
  ...
  for (var i = 0; i < node.content.length; ++i) {
    ...
    if (child.type === 'declaration') {
      ...
      lastDeclaration.declarationRange =
          WebInspector.TextRange.createFromLocation(child.start.line - 1, child.start.column - 1);
    }
