...
WebInspector.ResourceLoader.load = function(url, headers, callback) {
   ...
   * @param {!Object.<string, string>} headers
   */
  function mycallback(statusCode, headers) { callback(statusCode, headers, stream.data()); }
};
...
WebInspector.ResourceLoader.loadAsStream = function(url, headers, stream, callback) {
  ...
  InspectorFrontendHost.loadNetworkResource(
  ...
  function dataURLDecodeSuccessful(text) {
    ...
    finishedCallback(
        /** @type {!InspectorFrontendHostAPI.LoadNetworkResourceResult} */ ({statusCode: 200}));
  }

  function dataURLDecodeFailed() {
    finishedCallback(
        /** @type {!InspectorFrontendHostAPI.LoadNetworkResourceResult} */ ({statusCode: 404}));
  }
};
