...
WebInspector.TransformController = function(element, disableRotate) {
  ...
  WebInspector.installDragHandle(
      element, this._onDragStart.bind(this), this._onDrag.bind(this), this._onDragEnd.bind(this),
      'move', null);
  ...
  if (!disableRotate) {
    var panModeButton =
        new WebInspector.ToolbarToggle(WebInspector.UIString('Pan mode (X)'), 'pan-toolbar-item');
    panModeButton.addEventListener(
        'click', this._setMode.bind(this, WebInspector.TransformController.Modes.Pan));
    ...
    var rotateModeButton = new WebInspector.ToolbarToggle(
        WebInspector.UIString('Rotate mode (V)'), 'rotate-toolbar-item');
  ...
  var resetButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Reset transform (0)'), 'center-toolbar-item');
...
WebInspector.TransformController.prototype = {
  /**
     ...
     */
  toolbar: function() { return this._controlPanelToolbar; },
  ...
  _registerShortcuts: function() {
    ...
    this._addShortcuts(
        ...
        this._onKeyboardZoom.bind(this, zoomFactor));
    this._addShortcuts(
        WebInspector.ShortcutsScreen.LayersPanelShortcuts.ZoomOut,
        this._onKeyboardZoom.bind(this, 1 / zoomFactor));
    this._addShortcuts(
        WebInspector.ShortcutsScreen.LayersPanelShortcuts.Up,
        this._onKeyboardPanOrRotate.bind(this, 0, -1));
    this._addShortcuts(
        WebInspector.ShortcutsScreen.LayersPanelShortcuts.Down,
        this._onKeyboardPanOrRotate.bind(this, 0, 1));
    this._addShortcuts(
        WebInspector.ShortcutsScreen.LayersPanelShortcuts.Left,
        this._onKeyboardPanOrRotate.bind(this, -1, 0));
    this._addShortcuts(
  ...
  _toggleMode: function() {
    this._setMode(
        this._mode === WebInspector.TransformController.Modes.Pan ?
            WebInspector.TransformController.Modes.Rotate :
  ...
  /**
     * @return {number}
     */
  rotateX: function() { return this._rotateX; },

  /**
     * @return {number}
     */
  rotateY: function() { return this._rotateY; },
  ...
  _onScale: function(scaleFactor, x, y) {
    scaleFactor =
        Number.constrain(this._scale * scaleFactor, this._minScale, this._maxScale) / this._scale;
  ...
  _onKeyboardPanOrRotate: function(xMultiplier, yMultiplier) {
    ...
    if (this._mode === WebInspector.TransformController.Modes.Rotate) {
      // Sic! _onRotate treats X and Y as "rotate around X" and "rotate around Y", so swap X/Y
      // multiplers.
      this._onRotate(
          this._rotateX + yMultiplier * rotateStepInDegrees,
          this._rotateY + xMultiplier * rotateStepInDegrees);
    } else {
  ...
  _onMouseWheel: function(event) {
    ...
    this._onScale(
        scaleFactor, event.clientX - this.element.totalOffsetLeft(),
