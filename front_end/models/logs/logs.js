...
// ../../front_end/models/logs/LogManager.ts
...
// ../../front_end/models/logs/NetworkLog.ts
...
var NetworkLog = class _NetworkLog extends Common.ObjectWrapper.ObjectWrapper {
  ...
  static initiatorInfoForRequest(request, existingInitiatorData) {
    ...
    if (redirectSource) {
      ...
      if (initiator.type === Network.InitiatorType.Parser) {
        type = SDK.NetworkRequest.InitiatorType.PARSER;
        url = initiator.url ? initiator.url : url;
        lineNumber = initiator.lineNumber;
        columnNumber = initiator.columnNumber;
      } else if (initiator.type === Network.InitiatorType.Script) {
      ...
      } else if (initiator.type === Network.InitiatorType.Preload) {
        type = SDK.NetworkRequest.InitiatorType.PRELOAD;
      } else if (initiator.type === Network.InitiatorType.Preflight) {
        type = SDK.NetworkRequest.InitiatorType.PREFLIGHT;
        initiatorRequest = request.preflightInitiatorRequest();
      } else if (initiator.type === Network.InitiatorType.SignedExchange) {
  ...
  networkMessageGenerated(networkManager, event) {
    ...
    const consoleMessage = new SDK.ConsoleModel.ConsoleMessage(
      ...
      warning ? Log.LogEntryLevel.Warning : Log.LogEntryLevel.Info,
      message
    );
...
var LogManager = class _LogManager {
  ...
  modelAdded(logModel) {
    const eventListeners = [];
    eventListeners.push(logModel.addEventListener(SDK2.LogModel.Events.ENTRY_ADDED, this.logEntryAdded, this));
  ...
  logEntryAdded(event) {
    ...
    const consoleMessage = new SDK2.ConsoleModel.ConsoleMessage(
      target.model(SDK2.RuntimeModel.RuntimeModel),
      entry.source,
      entry.level,
      entry.text,
      details
    ...
    if (consoleMessage.source === Log.LogEntrySource.Worker) {
...
// ../../front_end/models/logs/RequestResolver.ts
...
var RequestResolver = class extends Common3.ResolverBase.ResolverBase {
  ...
  startListening() {
    ...
    this.networkListener = this.networkLog.addEventListener("RequestAdded" /* RequestAdded */, this.onRequestAdded, this);
  }
  stopListening() {
    if (!this.networkListener) {
      return;
