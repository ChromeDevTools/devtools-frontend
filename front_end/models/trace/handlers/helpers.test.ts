...
describeWithEnvironment('getNonResolvedURL', () => {
  it('returns the URL in event.args.data if it has one', async function() {
    ...
    const commitLoadEvent = Trace.Extras.AllThreadEntries.forTrace(parsedTrace).find(Trace.Types.Events.isCommitLoad);
  ...
  it('returns the URL for a ProfileCall from the callframe', async function() {
    const {parsedTrace} = await TraceLoader.traceEngine(this, 'web-dev-with-commit.json.gz');
    const profileCall = Trace.Extras.AllThreadEntries.forTrace(parsedTrace).find(Trace.Types.Events.isProfileCall);
  ...
  it('parses out the URL For a ParseAuthorStyleSheet', async function() {
    const {parsedTrace} = await TraceLoader.traceEngine(this, 'web-dev-with-commit.json.gz');
    const parseStyle =
        Trace.Extras.AllThreadEntries.forTrace(parsedTrace).find(Trace.Types.Events.isParseAuthorStyleSheetEvent);
  ...
  it('for a generic event with a stackTrace property, it uses the URL of the top frame', async function() {
    const {parsedTrace} = await TraceLoader.traceEngine(this, 'web-dev-with-commit.json.gz');
    const eventDispatch = Trace.Extras.AllThreadEntries.forTrace(parsedTrace).find(entry => {
  ...
  it('uses the PaintImage URL for a DecodeImage event', async function() {
    ...
    const decodeImage = Trace.Extras.AllThreadEntries.forTrace(parsedTrace).find(Trace.Types.Events.isDecodeImage);
