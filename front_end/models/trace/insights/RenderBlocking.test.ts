...
describe('RenderBlockingRequests', function() {
  ...
  it('considers only the navigation specified by the context', async () => {
    ...
    assert(
        insight.renderBlockingRequests.every(r => r.args.data.syntheticData.sendStartTime > navigations[1].ts),
        'a result is not contained by the nav bounds');
    assert(
        insight.renderBlockingRequests.every(r => r.args.data.syntheticData.finishTime < navigations[2].ts),
        'a result is not contained by the nav bounds');
  });
