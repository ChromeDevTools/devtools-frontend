...
WebInspector.BlockedURLsPane = function() {
  ...
  var addButton =
      new WebInspector.ToolbarButton(WebInspector.UIString('Add pattern'), 'add-toolbar-item');
  addButton.addEventListener('click', this._addButtonClicked.bind(this));
  this._toolbar.appendToolbarItem(addButton);
  var clearButton =
      new WebInspector.ToolbarButton(WebInspector.UIString('Remove all'), 'clear-toolbar-item');
...
WebInspector.BlockedURLsPane.prototype = {
  ...
  _emptyElementContextMenu: function(event) {
    ...
    contextMenu.appendItem(
        WebInspector.UIString.capitalize('Add ^pattern'), this._addButtonClicked.bind(this));
    contextMenu.show();
  },
  ...
  _edit: function(content, element, onAccept) {
    ...
    input.value = content;
    input.placeholder =
        WebInspector.UIString('Text pattern to block matching requests; use * for wildcard');
  ...
  _contextMenu: function(index, event) {
    ...
    contextMenu.appendItem(
        WebInspector.UIString.capitalize('Remove ^all'), this._removeAll.bind(this));
    contextMenu.show();
  },
  ...
  _createElement: function(url, index) {
    ...
    countElement.title = WebInspector.UIString(
        count === 1 ? '%d request blocked by this pattern' : '%d requests blocked by this pattern',
        count);
    ...
    element.addEventListener(
        'dblclick', this._edit.bind(this, url, element, this._changeBlockedURL.bind(this, index)),
        false);
    return element;
  },
