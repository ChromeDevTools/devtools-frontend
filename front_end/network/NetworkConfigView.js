...
WebInspector.NetworkConfigView.prototype = {
  ...
  _createCacheSection: function() {
    var section =
        this._createSection(WebInspector.UIString('Caching'), 'network-config-disable-cache');
  ...
  _createNetworkThrottlingSection: function() {
    var section = this._createSection(
        WebInspector.UIString('Network throttling'), 'network-config-throttling');
...
WebInspector.NetworkConfigView.createUserAgentSelectAndInput = function() {
  ...
  for (var userAgentDescriptor of groups) {
    ...
    for (var userAgentVersion of userAgentDescriptor.values) {
      var userAgentValue = WebInspector.MultitargetNetworkManager.patchUserAgentWithChromeVersion(
