 ...
 */
WebInspector.NetworkManageCustomHeadersView = function(
    columnData, addHeaderColumnCallback, changeHeaderColumnCallback, removeHeaderColumnCallback) {
  ...
  this.contentElement.appendChild(createTextButton(
      WebInspector.UIString('Add custom header\u2026'), this._addButtonClicked.bind(this),
      'add-button'));

  /** @type {!Map.<string, !{title: string, editable: boolean}>} */
  this._columnConfigs = new Map();
  columnData.forEach(
      columnData => this._columnConfigs.set(columnData.title.toLowerCase(), columnData));
...
WebInspector.NetworkManageCustomHeadersView.prototype = {
   ...
   * @override
   */
  wasShown: function() { this._headersUpdated(); },

  _headersUpdated: function() {
    this._list.clear();
    this._columnConfigs.forEach(
        headerData => this._list.appendItem({header: headerData.title}, headerData.editable));
  },

  _addButtonClicked: function() { this._list.addNewItem(this._columnConfigs.size, {header: ''}); },
  ...
  _createEditor: function() {
    ...
    titles.createChild('div', 'custom-headers-header').textContent =
        WebInspector.UIString('Header Name');

    var fields = content.createChild('div', 'custom-headers-edit-row');
    fields.createChild('div', 'custom-headers-header')
        .appendChild(
            editor.createInput('header', 'text', 'x-custom-header', validateHeader.bind(this)));
