...
WebInspector.NetworkOverview = function() {
  ...
  WebInspector.targetManager.addModelListener(
      WebInspector.ResourceTreeModel, WebInspector.ResourceTreeModel.Events.Load,
...
WebInspector.NetworkOverview.prototype = {
   ...
   * @override
   */
  wasShown: function() { this.onResize(); },
  ...
  update: function() {
    ...
    var newBoundary = new WebInspector.NetworkTimeBoundary(
        this._calculator.minimumBoundary(), this._calculator.maximumBoundary());
    if (!this._lastBoundary || !newBoundary.equals(this._lastBoundary)) {
      var span = this._calculator.boundarySpan();
      while (this._span < span)
        this._span *= 1.25;
      this._calculator.setBounds(
          this._calculator.minimumBoundary(), this._calculator.minimumBoundary() + this._span);
      this._lastBoundary = new WebInspector.NetworkTimeBoundary(
          this._calculator.minimumBoundary(), this._calculator.maximumBoundary());
    ...
    for (var i = 0; i < n; ++i) {
      ...
      var timeRanges = WebInspector.RequestTimingView.calculateRequestTimeRanges(
          request, this._calculator.minimumBoundary());
