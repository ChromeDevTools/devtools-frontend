...
WebInspector.XMLView.Node.populate = function(root, xmlNode, xmlView) {
  ...
  while (node) {
    ...
    if (nodeType === 3 && currentNode.nodeValue.match(/\s+/))
      continue;
    // ignore ATTRIBUTE, ENTITY_REFERENCE, ENTITY, DOCUMENT, DOCUMENT_TYPE, DOCUMENT_FRAGMENT,
    // NOTATION
...
WebInspector.XMLView.Node.prototype = {
  ...
  _updateTitle: function() {
    ...
    switch (node.nodeType) {
      case 1:  // ELEMENT
        ...
        if (!this.expanded) {
          if (node.childElementCount) {
            titleItems.push(
                '>', 'shadow-xml-view-tag', '\u2026', 'shadow-xml-view-comment', '</' + tag,
                'shadow-xml-view-tag');
      ...
      case 4:  // CDATA
        this._setTitle([
          '<![CDATA[', 'shadow-xml-view-cdata', node.nodeValue, 'shadow-xml-view-text', ']]>',
