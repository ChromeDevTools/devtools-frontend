...
const UIStrings = {
   ...
   */
  appIdNote:
      'Note: `id` isn’t specified in the manifest, `start_url` is used instead. To specify an app ID that matches the current identity, set the `id` field to {PH1} {PH2}.',
  ...
  manifestDoesNotContainASuitable:
      'Manifest doesn’t contain an icon that fits—PNG, SVG, or WebP format of at least {PH1}px is required, the \'`sizes`\' attribute must be set, and the \'`purpose`\' attribute, if set, must include \'`any`\'',
  /**
  ...
  pageDoesNotWorkOfflineThePage:
      'Page doesn’t work offline. Starting in Chrome 93, the installability criteria are changing, and this site won’t be installable. See {PH1} for more information.',
  /**
  ...
  actualSizeSspxOfSSDoesNotMatch:
      'Actual size ({PH1}×{PH2})px of {PH3} {PH4} doesn’t match specified size ({PH5}×{PH6}px)',
  /**
