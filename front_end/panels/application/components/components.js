...
// ../../front_end/panels/application/components/AdsView.ts
var AdsView_exports = {};
...
function maybeRenderExplanations(explanations, explanationTree, reasonToFramesMap) {
  ...
  const pageSupportNeeded = explanations.filter(
    (explanation) => explanation.type === Page.BackForwardCacheNotRestoredReasonType.PageSupportNeeded
  );
  const supportPending = explanations.filter(
    (explanation) => explanation.type === Page.BackForwardCacheNotRestoredReasonType.SupportPending
  );
  const circumstantial = explanations.filter(
    (explanation) => explanation.type === Page.BackForwardCacheNotRestoredReasonType.Circumstantial
  );
  return html2`
...
function maybeRenderReasonContext(explanation) {
  if (explanation.reason === Page.BackForwardCacheNotRestoredReason.EmbedderExtensionSentMessageToCachedFrame && explanation.context) {
...
function maybeRenderDeepLinkToUnload(explanation) {
  if (explanation.reason === Page.BackForwardCacheNotRestoredReason.UnloadHandlerExistsInMainFrame || explanation.reason === Page.BackForwardCacheNotRestoredReason.UnloadHandlerExistsInSubFrame) {
    return html2`
...
var BackForwardCacheView = class extends UI2.Widget.Widget {
  ...
  wasShown() {
    ...
    SDK2.TargetManager.TargetManager.instance().addModelListener(
      SDK2.ResourceTreeModel.ResourceTreeModel,
      SDK2.ResourceTreeModel.Events.BackForwardCacheDetailsUpdated,
      this.requestUpdate,
      this
    );
    this.requestUpdate();
  ...
  #renderBackForwardCacheTestResult() {
    SDK2.TargetManager.TargetManager.instance().removeModelListener(
      SDK2.ResourceTreeModel.ResourceTreeModel,
      SDK2.ResourceTreeModel.Events.FrameNavigated,
...
var renderDeletedSitesOrNoSitesMessage = (input) => {
  ...
  if (input.trackingSites.length === 0) {
    return html3`
      <devtools-report-section>
      ${input.screenStatus === "Running" /* RUNNING */ ? html3`
...
// ../../front_end/panels/application/components/CrashReportContextGrid.ts
...
function renderDisallowed(data, showDetails, onToggleShowDetails, onRevealDOMNode, onRevealHeader) {
  ...
  const featureRows = data.map(({ policy, blockReason, linkTargetDOMNode, linkTargetRequest }) => {
    const blockReasonText = (() => {
      switch (blockReason) {
        case Page.PermissionsPolicyBlockReason.IframeAttribute:
        ...
        case Page.PermissionsPolicyBlockReason.InFencedFrameTree:
...
var PermissionsPolicySection = class extends UI6.Widget.Widget {
  ...
  async performUpdate() {
    ...
    const disallowedData = this.#showDetails ? await Promise.all(disallowed.map(async (policy) => {
      ...
      const linkTargetDOMNode = await (blockReason === Page.PermissionsPolicyBlockReason.IframeAttribute && frame?.getOwnerDOMNodeOrDocument() || void 0);
      const resource = frame?.resourceForURL(frame.url);
      const linkTargetRequest = blockReason === Page.PermissionsPolicyBlockReason.Header && resource?.request || void 0;
...
function renderProtocolTest(protocolHandlers, queryInputState, protocolSelectHandler, queryInputChangeHandler, testProtocolClickHandler) {
  ...
  return html7`
    <div class="protocol-handlers-row">
      ...
      <devtools-button .variant=${Buttons4.Button.Variant.PRIMARY} @click=${testProtocolClickHandler}>
...
// ../../front_end/panels/application/components/ServiceWorkerRouterView.ts
...
var StorageMetadataView = class extends LegacyWrapper.LegacyWrapper.WrappableComponent {
  ...
  async renderReportContent() {
    ...
    const origin = this.#storageKey.origin;
    const ancestorChainHasCrossSite = Boolean(this.#storageKey.components.get(SDK5.StorageKeyManager.StorageKeyComponent.ANCESTOR_CHAIN_BIT));
    const hasNonce = Boolean(this.#storageKey.components.get(SDK5.StorageKeyManager.StorageKeyComponent.NONCE_HIGH));
    const topLevelSiteIsOpaque = Boolean(
      this.#storageKey.components.get(SDK5.StorageKeyManager.StorageKeyComponent.TOP_LEVEL_SITE_OPAQUE_NONCE_HIGH)
    );
    const topLevelSite = this.#storageKey.components.get(SDK5.StorageKeyManager.StorageKeyComponent.TOP_LEVEL_SITE);
  ...
  #renderStorageBucketInfo() {
    ...
    const renderBucketName = () => {
      ...
      const revealBucket = (e) => {
        e.preventDefault();
        void Common4.Revealer.reveal(
          new StorageBucketRevealInfo(this.#storageBucket)
        );
      };
  ...
  async #deleteBucket() {
    ...
    const ok = await UI10.UIUtils.ConfirmDialog.show(
      i18nString9(UIStrings10.bucketWillBeRemoved),
      i18nString9(UIStrings10.confirmBucketDeletion, { PH1: this.#storageBucket.bucket.name || "" }),
      this,
