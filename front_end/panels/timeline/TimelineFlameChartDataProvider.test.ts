...
describeWithEnvironment('TimelineFlameChartDataProvider', function() {
  ...
  it('shows initiator arrows when an event that has them is selected', async function() {
    ...
    const {data} = await TraceLoader.traceEngine(this, 'scheduler-post-task.json.gz');
    ...
    const event = allThreadEntriesInTrace(data).find(event => {
  ...
  describe('groupTreeEvents', function() {
    it('returns the correct events for tree views given a flame chart group', async function() {
      const dataProvider = new Timeline.TimelineFlameChartDataProvider.TimelineFlameChartDataProvider();
      const {data} = await TraceLoader.traceEngine(this, 'sync-like-timings.json.gz');
      const entityMapper = new Timeline.Utils.EntityMapper.EntityMapper(data);
      dataProvider.setModel(data, entityMapper);
  ...
  it('can return the FlameChart group for a given event', async function() {
    ...
    const longest = data.UserInteractions.longestInteractionEvent;
    assert.isOk(longest);
