...
describeWithEnvironment('Overlays', () => {
  ...
  it('can calculate the y position of a main chart event', async function() {
    ...
    overlays.updateVisibleWindow(parsedTrace.data.Meta.traceBounds);
  ...
  describe('rendering overlays', () => {
    ...
    it('only renders one TIMESTAMP_MARKER as it is a singleton', async function() {
      ...
      overlays.add({
        ...
        timestamp: parsedTrace.data.Meta.traceBounds.min,
      });
      overlays.add({
        type: 'TIMESTAMP_MARKER',
        timestamp: parsedTrace.data.Meta.traceBounds.max,
      });
      await overlays.update();
    ...
    it('shows correct tooltip on the `generate ai label` hover for the users with logging enabled', async function() {
      ...
      assert.strictEqual(
          ...
          'The selected call stack is sent to Google. The content you submit and that is generated by this feature will be used to improve Google’s AI models. This is an experimental AI feature and won’t always get it right. This data may be seen by human reviewers to improve this feature. Learn more in settings',
      );
    });
    ...
    it('shows correct tooltip text on `generate ai label` hover for the users with logging disabled', async function() {
      ...
      assert.strictEqual(
          cleanTextContent(tooltip.innerText),
          'The selected call stack is sent to Google. The content you submit and that is generated by this feature will not be used to improve Google’s AI models. This is an experimental AI feature and won’t always get it right. Your organization may change these settings at any time. Learn more in settings',
      );
    });
