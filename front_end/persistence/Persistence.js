...
WebInspector.Persistence = function(workspace, breakpointManager, fileSystemMapping) {
  ...
  this._eventListeners = [
    workspace.addEventListener(
        WebInspector.Workspace.Events.UISourceCodeAdded, this._onUISourceCodeAdded, this),
    workspace.addEventListener(
        WebInspector.Workspace.Events.UISourceCodeRemoved, this._onUISourceCodeRemoved, this),
    workspace.addEventListener(
        WebInspector.Workspace.Events.ProjectRemoved, this._onProjectRemoved, this),
...
WebInspector.Persistence._NodePrefix =
...
WebInspector.Persistence.prototype = {
  ...
  _createBinding: function(uiSourceCode) {
    if (uiSourceCode.project().type() === WebInspector.projectTypes.FileSystem) {
      var fileSystemPath =
          WebInspector.FileSystemWorkspaceBinding.fileSystemPath(uiSourceCode.project().id());
      var networkURL =
          this._fileSystemMapping.networkURLForFileSystemURL(fileSystemPath, uiSourceCode.url());
      var networkSourceCode = networkURL ? this._workspace.uiSourceCodeForURL(networkURL) : null;
      return networkSourceCode ?
          new WebInspector.PersistenceBinding(networkSourceCode, uiSourceCode) :
          null;
    }
    if (uiSourceCode.project().type() === WebInspector.projectTypes.Network) {
      ...
      var projectId =
          file ? WebInspector.FileSystemWorkspaceBinding.projectId(file.fileSystemPath) : null;
      var fileSourceCode =
          file && projectId ? this._workspace.uiSourceCode(projectId, file.fileURL) : null;
      return fileSourceCode ? new WebInspector.PersistenceBinding(uiSourceCode, fileSourceCode) :
                              null;
    }
    return null;
  },
  ...
  _bind: function(uiSourceCode) {
    console.assert(
        !uiSourceCode[WebInspector.Persistence._binding],
        'Cannot bind already bound UISourceCode!');
    ...
    if (binding.network.isDirty() || binding.fileSystem.isDirty()) {
      WebInspector.console.log(WebInspector.UIString(
          '%s can not be persisted to file system due to unsaved changes.',
          binding.network.name()));
      return;
    }
  ...
  _syncNodeJSContent: function(binding, uiSourceCode, currentContent, newContent) {
    if (uiSourceCode === binding.fileSystem) {
      ...
      if (currentContent.startsWith(WebInspector.Persistence._NodePrefix) &&
        ...
        newContent = WebInspector.Persistence._NodePrefix + newContent +
            WebInspector.Persistence._NodeSuffix;
    }
    binding[WebInspector.Persistence._muteCommit] = true;
  ...
  _moveBreakpoints: function(from, to) {
    ...
    for (var breakpoint of breakpoints) {
      ...
      this._breakpointManager.setBreakpoint(
          to, breakpoint.lineNumber(), breakpoint.columnNumber(), breakpoint.condition(),
          breakpoint.enabled());
    }
  },
