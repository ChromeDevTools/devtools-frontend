...
WebInspector.ProfileView = function(nodeFormatter, viewTypes) {
  ...
  this._viewType =
      WebInspector.settings.createSetting('profileView', WebInspector.ProfileView.ViewTypes.Heavy);
  ...
  columns.push(
      {id: 'function', title: WebInspector.UIString('Function'), disclosure: true, sortable: true});

  this.dataGrid = new WebInspector.DataGrid(columns);
  this.dataGrid.addEventListener(
      WebInspector.DataGrid.Events.SortingChanged, this._sortProfile, this);
  this.dataGrid.addEventListener(
      WebInspector.DataGrid.Events.SelectedNode, this._nodeSelected.bind(this, true));
  this.dataGrid.addEventListener(
      WebInspector.DataGrid.Events.DeselectedNode, this._nodeSelected.bind(this, false));
  ...
  var options = new Map(viewTypes.map(
      type => [type, this.viewSelectComboBox.createOption(optionNames.get(type), '', type)]));
  ...
  this.focusButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Focus selected function'), 'visibility-toolbar-item');
  this.focusButton.setEnabled(false);
  this.focusButton.addEventListener('click', this._focusClicked, this);

  this.excludeButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Exclude selected function'), 'delete-toolbar-item');
  this.excludeButton.setEnabled(false);
  this.excludeButton.addEventListener('click', this._excludeClicked, this);

  this.resetButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Restore all functions'), 'refresh-toolbar-item');
...
WebInspector.ProfileView.prototype = {
  ...
  /**
     ...
     * @return {string}
     */
  columnHeader: function(columnId) { throw 'Not implemented'; },

  /**
     * @return {?WebInspector.Target}
     */
  target: function() { return this._profileHeader.target(); },
   ...
   * @override
   */
  willHide: function() { this._currentSearchResultIndex = -1; },

  refresh: function() {
    var selectedProfileNode =
        this.dataGrid.selectedNode ? this.dataGrid.selectedNode.profileNode : null;
   ...
   * @override
   */
  searchCanceled: function() { this._searchableElement.searchCanceled(); },
   ...
   * @override
   */
  jumpToNextSearchResult: function() { this._searchableElement.jumpToNextSearchResult(); },

  /**
   * @override
   */
  jumpToPreviousSearchResult: function() { this._searchableElement.jumpToPreviousSearchResult(); },

  /**
     * @return {!WebInspector.Linkifier}
     */
  linkifier: function() { return this._linkifier; },

  /**
     * @return {!WebInspector.FlameChartDataProvider}
     */
  createFlameChartDataProvider: function() { throw 'Not implemented'; },
  ...
  _ensureFlameChartCreated: function() {
    ...
    this._flameChart =
        new WebInspector.CPUProfileFlameChart(this._searchableView, this._dataProvider);
    this._flameChart.addEventListener(
        WebInspector.FlameChart.Events.EntrySelected, this._onEntrySelected.bind(this));
  },
  ...
  _sortProfile: function() {
    ...
    var sortProperty =
        sortColumnIdentifier === 'function' ? 'functionName' : sortColumnIdentifier || '';
    this.profileDataGridTree.sort(
        WebInspector.ProfileDataGridTree.propertyComparator(sortProperty, sortAscending));
...
WebInspector.WritableProfileHeader = function(target, type, title) {
  WebInspector.ProfileHeader.call(
      this, target, type, title || WebInspector.UIString('Profile %d', type.nextProfileUid()));
...
WebInspector.WritableProfileHeader.prototype = {
  ...
  onTransferStarted: function() {
    ...
    this.updateStatus(
        WebInspector.UIString(
            'Loading\u2026 %s', Number.bytesToString(this._jsonifiedProfile.length)),
        true);
  },
  ...
  onChunkTransferred: function(reader) {
    this.updateStatus(WebInspector.UIString(
        'Loading\u2026 %d%%', Number.bytesToString(this._jsonifiedProfile.length)));
  },
   ...
   * @param {string} text
   */
  write: function(text) { this._jsonifiedProfile += text; },
   ...
   * @override
   */
  dispose: function() { this.removeTempFile(); },
  ...
  /**
     ...
     * @return {boolean}
     */
  canSaveToFile: function() { return !this.fromFile() && this._protocolProfile; },
  ...
  saveToFile: function() {
    ...
    this._fileName = this._fileName ||
        `${this._profileType.typeName()}-${new Date()
            .toISO8601Compact()}${this._profileType.fileExtension()}`;
  ...
  _saveProfileDataToTempFile: function(data) {
     ...
     * @this {WebInspector.WritableProfileHeader}
     */
    function didCreateTempFile(tempFile) { this._writeToTempFile(tempFile, serializedData); }
