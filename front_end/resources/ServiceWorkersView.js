...
WebInspector.ServiceWorkersView.prototype = {
  ...
  targetAdded: function(target) {
    ...
    this._showAllCheckbox = new WebInspector.ToolbarCheckbox(
        ...
        WebInspector.UIString('Show all Service Workers regardless of the origin'));
    ...
    this._manager.addEventListener(
        WebInspector.ServiceWorkerManager.Events.RegistrationUpdated, this._registrationUpdated,
        this);
    this._manager.addEventListener(
        WebInspector.ServiceWorkerManager.Events.RegistrationDeleted, this._registrationDeleted,
        this);
    this._manager.addEventListener(
  ...
  _registrationErrorAdded: function(event) {
    var registration =
        /** @type {!WebInspector.ServiceWorkerRegistration} */ (event.data['registration']);
 ...
 * @param {!WebInspector.ServiceWorkerRegistration} registration
 */
WebInspector.ServiceWorkersView.Section = function(
    manager, subTargetsManager, section, registration) {
  ...
  this._updateButton = new WebInspector.ToolbarButton(
      WebInspector.UIString('Update'), undefined, WebInspector.UIString('Update'));
...
WebInspector.ServiceWorkersView.Section.prototype = {
  ...
  _update: function() {
    ...
    if (active) {
      ...
      scriptElement.createChild('div', 'report-field-value-subtitle').textContent =
          WebInspector.UIString(
              'Received %s', new Date(active.scriptResponseTime * 1000).toLocaleString());
      ...
      if (active.isRunning() || active.isStarting()) {
        ...
        createLink(
            activeEntry, WebInspector.UIString('start'), this._startButtonClicked.bind(this));
      }

      var clientsList =
          this._wrapWidget(this._section.appendField(WebInspector.UIString('Clients')));
      ...
      for (var client of active.controlledClients) {
        ...
        if (this._clientInfoCache.has(client))
          this._updateClientInfo(
              clientLabelText,
              /** @type {!WebInspector.TargetInfo} */ (this._clientInfoCache.get(client)));
    ...
    if (waiting) {
      ...
      createLink(
          waitingEntry, WebInspector.UIString('skipWaiting'), this._skipButtonClicked.bind(this));
    ...
    this._moreButton = createLink(
        errorsValue,
        this._errorsList.classList.contains('hidden') ? WebInspector.UIString('details') :
                                                        WebInspector.UIString('hide'),
        this._moreErrorsButtonClicked.bind(this));
    createLink(
        errorsValue, WebInspector.UIString('clear'), this._clearErrorsButtonClicked.bind(this));
  ...
  _addError: function(error) {
    ...
    message.appendChild(
        this._linkifier.linkifyScriptLocation(target, null, error.sourceURL, error.lineNumber));
    message.appendChild(
        createLabel('#' + error.versionId + ': ' + error.errorMessage, 'error-icon'));
  },

  _unregisterButtonClicked: function() { this._manager.deleteRegistration(this._registration.id); },

  _updateButtonClicked: function() { this._manager.updateRegistration(this._registration.id); },
   ...
   */
  _activateTarget: function(targetId) { this._subTargetsManager.activateTarget(targetId); },

  _startButtonClicked: function() { this._manager.startWorker(this._registration.scopeURL); },

  _skipButtonClicked: function() { this._manager.skipWaiting(this._registration.scopeURL); },

  /**
   * @param {string} versionId
   */
  _stopButtonClicked: function(versionId) { this._manager.stopWorker(versionId); },

  _moreErrorsButtonClicked: function() {
    var newVisible = this._errorsList.classList.contains('hidden');
    this._moreButton.textContent =
        newVisible ? WebInspector.UIString('hide') : WebInspector.UIString('details');
   ...
   * @param {string} versionId
   */
  _inspectButtonClicked: function(versionId) { this._manager.inspectWorker(versionId); },
