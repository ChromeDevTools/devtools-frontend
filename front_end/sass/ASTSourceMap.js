 ...
 * @param {?function(!WebInspector.ASTSourceMap, !Array<!WebInspector.TextRange>,
...
WebInspector.ASTSourceMap.prototype = {
  ...
  /**
     ...
     * @return {string}
     */
  url: function() { return this._sourceMapURL; },
  ...
  findEntry: function(lineNumber, columnNumber) {
    ...
    return new WebInspector.SourceMapEntry(
        lineNumber, columnNumber, sourceNode.document.url, sourceNode.range.startLine,
        sourceNode.range.startColumn);
  },
  ...
  /**
     ...
     * @return {boolean}
     */
  editable: function() { return !!this._editCallback; },
  ...
  sourceModels: function() {
    var sourceModels =
        /** @type {!Map<string, !WebInspector.SASSSupport.AST>} */ (new Map(this._models));
  ...
  /**
     ...
     * @return {?WebInspector.SASSSupport.AST}
     */
  modelForURL: function(url) { return this._models.get(url) || null; },
  ...
  /**
     ...
     */
  toSourceNode: function(compiled) { return this._compiledToSource.get(compiled) || null; },
  ...
  rebase: function(updated, outNodeMapping) {
    ...
    var newMap = new WebInspector.ASTSourceMap(
        this._compiledURL, this._sourceMapURL, models, this._editCallback);
    var compiledNodes = this._compiledToSource.keysArray();
    for (var i = 0; i < compiledNodes.length; ++i) {
      var compiledNode = compiledNodes[i];
      var sourceNode = /** @type {!WebInspector.SASSSupport.TextNode} */ (
          this._compiledToSource.get(compiledNode));
      var mappedCompiledNode = /** @type {!WebInspector.SASSSupport.TextNode} */ (
          outNodeMapping.get(compiledNode) || compiledNode);
      var mappedSourceNode = /** @type {!WebInspector.SASSSupport.TextNode} */ (
