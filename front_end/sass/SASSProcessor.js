...
WebInspector.SASSProcessor.processCSSEdits = function(astService, map, ranges, newTexts) {
  ...
  function onCSSParsed(newCSSAST) {
    ...
    var nodeMapping = new Map();
    var rebasedMap =
        /** @type {!WebInspector.ASTSourceMap} */ (map.rebase(clonedModels, nodeMapping));
...
WebInspector.SASSProcessor.EditOperation.prototype = {
  /**
     ...
     * @return {boolean}
     */
  merge: function(other) { return false; },

  /**
     * @return {!Array<!WebInspector.SASSSupport.Rule>}
     */
  perform: function() { return []; },
  ...
  /**
     ...
     */
  rebase: function(newMap, nodeMapping) { return this; },
};
...
WebInspector.SASSProcessor.SetTextOperation.fromCSSChange = function(change, map) {
  ...
  console.assert(
      oldProperty && newProperty, 'SetTextOperation must have both oldProperty and newProperty');
  var newValue = null;
  var sassNode = null;
...
WebInspector.SASSProcessor.SetTextOperation.prototype = {
  ...
  rebase: function(newMap, nodeMapping) {
    var sassNode =
        /** @type {?WebInspector.SASSSupport.TextNode} */ (nodeMapping.get(this._sassNode)) ||
        this._sassNode;
...
WebInspector.SASSProcessor.TogglePropertyOperation.prototype = {
  ...
  rebase: function(newMap, nodeMapping) {
    var sassProperty =
        /** @type {?WebInspector.SASSSupport.Property} */ (nodeMapping.get(this._sassProperty)) ||
        this._sassProperty;
    return new WebInspector.SASSProcessor.TogglePropertyOperation(
        newMap, sassProperty, this._newDisabled);
  },
 ...
 */
WebInspector.SASSProcessor.InsertPropertiesOperation = function(
    map, sassRule, afterSASSProperty, propertyNames, propertyValues, disabledStates) {
  console.assert(
      propertyNames.length === propertyValues.length &&
      propertyValues.length === disabledStates.length);
...
WebInspector.SASSProcessor.InsertPropertiesOperation.prototype = {
  ...
  perform: function() {
    ...
    if (this._afterSASSProperty) {
      afterCSSProperties =
          WebInspector.SASSProcessor._toCSSProperties(this.map, this._afterSASSProperty);
      cssRules = afterCSSProperties.map(property => property.parent);
    } else {
      cssRules =
          this.map.toCompiledNodes(this._sassRule.blockStart).map(blockStart => blockStart.parent);
    }
    for (var i = 0; i < cssRules.length; ++i) {
      var cssRule = cssRules[i];
      var afterCSSProperty = afterCSSProperties.length ? afterCSSProperties[i] : null;
      var newCSSProperties = cssRule.insertProperties(
          afterCSSProperty, this._nameTexts, this._valueTexts, this._disabledStates);
  ...
  rebase: function(newMap, nodeMapping) {
    var sassRule =
        /** @type {?WebInspector.SASSSupport.Rule} */ (nodeMapping.get(this._sassRule)) ||
        this._sassRule;
    var afterSASSProperty = this._afterSASSProperty ?
        /** @type {?WebInspector.SASSSupport.Property} */ (
            nodeMapping.get(this._afterSASSProperty)) ||
            this._afterSASSProperty :
        null;
    return new WebInspector.SASSProcessor.InsertPropertiesOperation(
        newMap, sassRule, afterSASSProperty, this._nameTexts, this._valueTexts,
