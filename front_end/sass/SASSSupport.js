...
WebInspector.SASSSupport.ASTDocument.prototype = {
  /**
     ...
     */
  clone: function() { return new WebInspector.SASSSupport.ASTDocument(this.url, this.text); },

  /**
     * @return {boolean}
     */
  hasChanged: function() { return !!this.edits.length; },
...
WebInspector.SASSSupport.Property.prototype = {
  ...
  clone: function(document) {
    return new WebInspector.SASSSupport.Property(
        document, this.name.clone(document), this.value.clone(document), this.range.clone(),
        this.disabled);
  },
  ...
  match: function(other, outNodeMapping) {
    ...
    if (outNodeMapping)
      outNodeMapping.set(this, other);
    return this.name.match(other.name, outNodeMapping) &&
        this.value.match(other.value, outNodeMapping);
  },
  ...
  setDisabled: function(disabled) {
    ...
    if (disabled) {
      var oldRange1 =
          WebInspector.TextRange.createFromLocation(this.range.startLine, this.range.startColumn);
      var edit1 = new WebInspector.SourceEdit(this.document.url, oldRange1, '/* ');
      var oldRange2 =
          WebInspector.TextRange.createFromLocation(this.range.endLine, this.range.endColumn);
    ...
    var oldRange1 = new WebInspector.TextRange(
        this.range.startLine, this.range.startColumn, this.range.startLine,
        this.name.range.startColumn);
    var edit1 = new WebInspector.SourceEdit(this.document.url, oldRange1, '');
    ...
    var oldRange2 = new WebInspector.TextRange(
        this.range.endLine, this.value.range.endColumn + (endsWithSemicolon ? 1 : 0),
  ...
  remove: function() {
    ...
    var oldRange;
    if (this.document.text.extract(lineRange).trim() ===
        this.document.text.extract(this.range).trim())
      oldRange = lineRange;
    else
      oldRange = this.range;
...
WebInspector.SASSSupport.Rule = function(document, selectors, styleRange, properties) {
  ...
  this.blockStart = new WebInspector.SASSSupport.TextNode(
      document, this.document.text.extract(blockStartRange), blockStartRange);
  this.blockStart.parent = this;

  for (var i = 0; i < this.properties.length; ++i)
    this.properties[i].parent = this;

  this._hasTrailingSemicolon = !this.properties.length ||
      this.document.text.extract(this.properties.peekLast().range).endsWith(';');
};
...
WebInspector.SASSSupport.Rule.prototype = {
  ...
  clone: function(document) {
    ...
    return new WebInspector.SASSSupport.Rule(
        document, selectors, this.styleRange.clone(), properties);
  },
  ...
  insertProperties: function(anchorProperty, nameTexts, valueTexts, disabledStates) {
    ...
    for (var i = 0; i < nameTexts.length; ++i) {
      ...
      this.document.edits.push(
          this._insertPropertyEdit(anchorProperty, nameText, valueText, disabled));
  ...
  _computePropertyIndent: function() {
    ...
    if (indentProperty) {
      result = this.document.text.extract(new WebInspector.TextRange(
          indentProperty.range.startLine, 0, indentProperty.range.startLine,
          indentProperty.range.startColumn));
    } else {
      var lineNumber = this.blockStart.range.startLine;
      var columnNumber = this.blockStart.range.startColumn;
      var baseLine = this.document.text.extract(
          new WebInspector.TextRange(lineNumber, 0, lineNumber, columnNumber));
      result = WebInspector.TextUtils.lineIndent(baseLine) +
...
WebInspector.SASSSupport.AST.prototype = {
  ...
  findNodeForPosition: function(lineNumber, columnNumber) {
    ...
    var index = this._sortedTextNodes.lowerBound(
        {lineNumber: lineNumber, columnNumber: columnNumber}, nodeComparator);
 ...
 */
WebInspector.SASSSupport.PropertyChange = function(
    type, oldRule, newRule, oldPropertyIndex, newPropertyIndex) {
  this.type = type;
...
WebInspector.SASSSupport.PropertyChange.prototype = {
  /**
     * @return {?WebInspector.SASSSupport.Property}
     */
  oldProperty: function() { return this.oldRule.properties[this.oldPropertyIndex] || null; },

  /**
     * @return {?WebInspector.SASSSupport.Property}
     */
  newProperty: function() { return this.newRule.properties[this.newPropertyIndex] || null; }
};
...
WebInspector.SASSSupport.diffModels = function(oldAST, newAST) {
  ...
  console.assert(
      oldAST.document.url === newAST.document.url,
      'Diff makes sense for models with the same url.');
  ...
  return new WebInspector.SASSSupport.ASTDiff(
      oldAST.document.url, oldAST, newAST, mapping, changes);
  ...
  function addChange(type, oldRule, newRule, oldPropertyIndex, newPropertyIndex) {
    changes.push(new WebInspector.SASSSupport.PropertyChange(
        type, oldRule, newRule, oldPropertyIndex, newPropertyIndex));
  }
  ...
  function computeRuleDiff(mapping, oldRule, newRule) {
    ...
    for (var i = 0; i < oldRule.properties.length; ++i)
      oldLines.push(
          oldRule.properties[i].name.text.trim() + ':' + oldRule.properties[i].value.text.trim());
    var newLines = [];
    for (var i = 0; i < newRule.properties.length; ++i)
      newLines.push(
          newRule.properties[i].name.text.trim() + ':' + newRule.properties[i].value.text.trim());
