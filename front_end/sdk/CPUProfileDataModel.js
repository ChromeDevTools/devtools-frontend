...
WebInspector.CPUProfileDataModel.prototype = {
  ...
  _compatibilityConversionHeadToNodes: function(profile) {
    ...
    function convertNodesTree(node) {
      nodes.push(node);
      node.children =
          (/** @type {!Array<!ProfilerAgent.ProfileNode>} */ (node.children)).map(convertNodesTree);
      return node.id;
    }
  },
  ...
  _translateProfileTree: function(nodes) {
    ...
    while (sourceNodeStack.length) {
      ...
      sourceNodeStack.push.apply(
          sourceNodeStack, sourceNode.children.map(id => nodeByIdMap.get(id)));
    }
    if (this.samples)
  ...
  _extractMetaNodes: function() {
    ...
    for (var i = 0; i < topLevelNodes.length && !(this.gcNode && this.programNode && this.idleNode);
         i++) {
   ...
   * @param {function(number, !WebInspector.CPUProfileNode, number, number, number)}
   * closeFrameCallback
  ...
  /**
     ...
     * @return {?WebInspector.CPUProfileNode}
     */
  nodeByIndex: function(index) { return this._idToNode.get(this.samples[index]) || null; },
