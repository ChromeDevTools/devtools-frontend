 ...
 */
WebInspector.CSSMatchedStyles = function(
    cssModel, node, inlinePayload, attributesPayload, matchedPayload, pseudoPayload,
  ...
  for (var i = 0; parentNode && inheritedPayload && i < inheritedPayload.length; ++i) {
    ...
    var inheritedInlineStyle = entryPayload.inlineStyle ?
        new WebInspector.CSSStyleDeclaration(
            cssModel, null, entryPayload.inlineStyle,
            WebInspector.CSSStyleDeclaration.Type.Inline) :
        null;
    ...
    for (var j = inheritedMatchedCSSRules.length - 1; j >= 0; --j) {
      ...
      addMatchingSelectors.call(
          this, parentNode, inheritedRule, inheritedMatchedCSSRules[j].matchingSelectors);
  ...
  if (animationsPayload)
    this._keyframes =
        animationsPayload.map(rule => new WebInspector.CSSKeyframesRule(cssModel, rule));
...
WebInspector.CSSMatchedStyles.prototype = {
  ...
  /**
     ...
     */
  cssModel: function() { return this._cssModel; },
  ...
  /**
     ...
     */
  nodeStyles: function() { return this._nodeStyles; },
  ...
  /**
     ...
     */
  pseudoStyles: function() { return this._pseudoStyles; },
  ...
  /**
     ...
     * @return {?WebInspector.DOMNode}
     */
  nodeForStyle: function(style) { return this._nodeForStyle.get(style) || null; },

  /**
     * @param {!WebInspector.CSSStyleDeclaration} style
     * @return {boolean}
     */
  isInherited: function(style) { return this._inheritedStyles.has(style); },
    ...
    for (var i = 0; i < styles.length; ++i) {
      ...
      for (var j = 0; j < allProperties.length; ++j) {
        ...
        if (property.important) {
          if (inherited && isKnownProperty &&
              inheritedFromNode !== inheritedPropertyToNode.get(canonicalName)) {
            result.set(property, WebInspector.CSSMatchedStyles.PropertyState.Overloaded);
            continue;
          }
      ...
      for (var property of style.leadingProperties()) {
        ...
        for (var longhand of longhands) {
          var longhandCanonicalName =
              WebInspector.cssMetadata().canonicalPropertyName(longhand.name);
