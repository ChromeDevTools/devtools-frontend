...
WebInspector.HeapProfilerModel.prototype = {
  ...
  stopSampling: function() {
    ...
    return this._heapProfilerAgent
        .stopSampling((error, profile) => { currentProfile = !error ? profile : null; })
        .then(() => currentProfile);
  },
...
WebInspector.HeapProfilerDispatcher.prototype = {
   ...
   * @param {!Array.<number>} samples
   */
  heapStatsUpdate: function(samples) { this._heapProfilerModel.heapStatsUpdate(samples); },
   ...
   * @param {string} chunk
   */
  addHeapSnapshotChunk: function(chunk) { this._heapProfilerModel.addHeapSnapshotChunk(chunk); },
