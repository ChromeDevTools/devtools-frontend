...
WebInspector.InspectorBackendHostedMode.generateCommands = function(schema) {
  ...
  for (var i = 0; i < domains.length; ++i) {
        ...
        for (var k = 0; k < properties.length; ++k) {
          ...
          if ((property['type'] === 'string') && property['enum'])
            result.push(generateEnum(
                domain.domain + '.' + type.id + property['name'].toTitleCase(), property['enum']));
        }
      }
    }
    ...
    for (var j = 0; j < commands.length; ++j) {
      ...
      result.push(
          'InspectorBackend.registerCommand("' + domain.domain + '.' + command.name + '", [' +
          paramsText.join(', ') + '], [' + returnsText.join(', ') + '], ' + hasErrorData + ');');
    }
