...
WebInspector.ServiceWorkerCacheModel.prototype = {
  ...
  deleteCache: function(cache) {
    ...
    function callback(error) {
      if (error) {
        console.error(
            'ServiceWorkerCacheAgent error deleting cache ', cache.toString(), ': ', error);
        return;
      }
  ...
  _addOrigin: function(securityOrigin) { this._loadCacheNames(securityOrigin); },
  ...
  _requestEntries: function(cache, skipCount, pageSize, callback) {
    ...
    function innerCallback(error, dataEntries, hasMore) {
      ...
      for (var i = 0; i < dataEntries.length; ++i) {
        entries.push(new WebInspector.ServiceWorkerCacheModel.Entry(
            dataEntries[i].request, dataEntries[i].response));
      }
...
WebInspector.ServiceWorkerCacheModel.Cache.prototype = {
  /**
     ...
     * @return {boolean}
     */
  equals: function(cache) { return this.cacheId === cache.cacheId; },

  /**
     * @override
     * @return {string}
     */
  toString: function() { return this.securityOrigin + this.cacheName; }
};
...
WebInspector.ServiceWorkerCacheModel.fromTarget = function(target) {
  if (!target.hasBrowserCapability())
    return null;
  var instance = /** @type {?WebInspector.ServiceWorkerCacheModel} */ (
      target.model(WebInspector.ServiceWorkerCacheModel));
  if (!instance)
    instance = new WebInspector.ServiceWorkerCacheModel(
