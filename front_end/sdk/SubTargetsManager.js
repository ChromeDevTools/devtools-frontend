...
WebInspector.SubTargetsManager.prototype = {
   ...
   */
  activateTarget: function(targetId) { this._agent.activateTarget(targetId); },
  ...
  /**
     ...
     */
  targetForId: function(targetId) { return this._attachedTargets.get(targetId) || null; },
  ...
  _attachedToTarget: function(targetId, waitingForDebugger) {
    ...
    var target = WebInspector.targetManager.createTarget(
        targetName, this._capabilitiesForType(targetInfo.type), connection, this.target());
    ...
    var mainIsServiceWorker = !this.target().parentTarget() &&
        this.target().hasWorkerCapability() && !this.target().hasBrowserCapability();
...
WebInspector.SubTargetsDispatcher.prototype = {
   ...
   * @param {string} targetId
   */
  targetRemoved: function(targetId) { this._manager._targetRemoved(targetId); },
   ...
   * @param {string} targetId
   */
  detachedFromTarget: function(targetId) { this._manager._detachedFromTarget(targetId); },
...
WebInspector.SubTargetConnection.prototype = {
  ...
  _close: function() { this.connectionClosed('target_terminated'); },
