...
WebInspector.TracingModel.isAsyncPhase = function(phase) {
  return WebInspector.TracingModel.isNestableAsyncPhase(phase) || phase === 'S' || phase === 'T' ||
      phase === 'F' || phase === 'p';
};
...
WebInspector.TracingModel._extractId = function(payload) {
  ...
  console.error(
      `Unexpected id2 field at ${payload.ts /
      1000}, one and only one of 'local' and 'global' should be present.`);
};
...
WebInspector.TracingModel.browserMainThread = function(tracingModel) {
  ...
  for (var process of processes) {
    ...
    crRendererMainThreads.push(
        ...process.sortedThreads().filter(t => t.name() === 'CrBrowserMain'));
  }
...
WebInspector.TracingModel.prototype = {
  /**
     ...
     */
  devToolsMetadataEvents: function() { return this._devToolsMetadataEvents; },
  ...
  _addEvent: function(payload) {
    ...
    if (!event)
      return;
    // Build async event when we've got events from all threads & processes, so we can sort them and
    // process in the
    // chronological order. However, also add individual async events to the thread flow (above), so
    // we can easily
  ...
  /**
     ...
     */
  processByName: function(name) { return this._processByName.get(name); },
    ...
    switch (event.phase) {
      ...
      case phase.NestableAsyncEnd:
        ...
        if (top.name !== event.name) {
          console.error(`Begin/end event mismatch for nestable async event, ${top.name
                        } vs. ${event.name}, key: ${key}`);
          break;
        }
        top._addStep(event);
    }
  ...
  _addAsyncEvent: function(event) {
    ...
    if (event.phase === phase.AsyncStepInto || event.phase === phase.AsyncStepPast) {
      ...
      if (lastStep.phase !== phase.AsyncBegin && lastStep.phase !== event.phase) {
        console.assert(
            false, 'Async event step phase mismatch: ' + lastStep.phase + ' at ' +
                lastStep.startTime + ' vs. ' + event.phase + ' at ' + event.startTime);
        return;
      }
      asyncEvent._addStep(event);
      return;
...
WebInspector.TracingModel.Event.fromPayload = function(payload, thread) {
  var event = new WebInspector.TracingModel.Event(
      payload.cat, payload.name, /** @type {!WebInspector.TracingModel.Phase} */ (payload.ph),
...
WebInspector.TracingModel.Event.prototype = {
  /**
     ...
     * @return {boolean}
     */
  hasCategory: function(categoryName) { return this._parsedCategories.has(categoryName); },
...
WebInspector.TracingModel.Event.compareStartAndEndTime = function(a, b) {
  return a.startTime - b.startTime ||
      (b.endTime !== undefined && a.endTime !== undefined && b.endTime - a.endTime) || 0;
};
...
WebInspector.TracingModel.ObjectSnapshot = function(category, name, startTime, thread) {
  WebInspector.TracingModel.Event.call(
      this, category, name, WebInspector.TracingModel.Phase.SnapshotObject, startTime, thread);
};
...
WebInspector.TracingModel.ObjectSnapshot.fromPayload = function(payload, thread) {
  var snapshot = new WebInspector.TracingModel.ObjectSnapshot(
      payload.cat, payload.name, payload.ts / 1000, thread);
  var id = WebInspector.TracingModel._extractId(payload);
  if (typeof id !== 'undefined')
    snapshot.id = id;
...
WebInspector.TracingModel.AsyncEvent = function(startEvent) {
  WebInspector.TracingModel.Event.call(
      this, startEvent.categoriesString, startEvent.name, startEvent.phase, startEvent.startTime,
      startEvent.thread);
...
WebInspector.TracingModel.NamedObject.prototype = {
   ...
   * @param {string} name
   */
  _setName: function(name) { this._name = name; },
   ...
   */
  _setSortIndex: function(sortIndex) { this._sortIndex = sortIndex; },
};
...
WebInspector.TracingModel.NamedObject._sort = function(array) {
  ...
  function comparator(a, b) {
    return a._sortIndex !== b._sortIndex ? a._sortIndex - b._sortIndex :
...
WebInspector.TracingModel.Process.prototype = {
  ...
  /**
     ...
     * @return {?WebInspector.TracingModel.Thread}
     */
  threadByName: function(name) { return this._threadByName.get(name) || null; },

  /**
   * @param {string} name
   * @param {!WebInspector.TracingModel.Thread} thread
   */
  _setThreadByName: function(name, thread) { this._threadByName.set(name, thread); },

  /**
     * @param {!WebInspector.TracingManager.EventPayload} payload
     * @return {?WebInspector.TracingModel.Event} event
     */
  _addEvent: function(payload) { return this.threadById(payload.tid)._addEvent(payload); },
...
WebInspector.TracingModel.Thread.prototype = {
      ...
      switch (e.phase) {
        case phases.End:
          ...
          if (top.name !== e.name || top.categoriesString !== e.categoriesString)
            console.error(
                'B/E events mismatch at ' + top.startTime + ' (' + top.name + ') vs. ' +
                e.startTime + ' (' + e.name + ')');
          else
            top._complete(e);
          break;
   ...
   */
  _addAsyncEvent: function(asyncEvent) { this._asyncEvents.push(asyncEvent); },
  ...
  /**
     * @return {!WebInspector.TracingModel.Process}
     */
  process: function() { return this._process; },
  ...
  /**
     ...
     */
  asyncEvents: function() { return this._asyncEvents; },
