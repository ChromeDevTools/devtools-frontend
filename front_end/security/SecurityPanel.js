...
WebInspector.SecurityPanel = function() {
  ...
  this._sidebarTree = new WebInspector.SecurityPanelSidebarTree(
      this._sidebarMainViewElement, this.showOrigin.bind(this));
 ...
 * @property {?NetworkAgent.SecurityDetails} securityDetails - Security details of the origin, if
 * available.
 * @property {?Promise<>} certificateDetailsPromise - Certificate details of the origin.
 * @property {?WebInspector.SecurityOriginView} originView - Current SecurityOriginView
...
WebInspector.SecurityPanel.prototype = {
   ...
   */
  _updateSecurityState: function(
      newSecurityState, explanations, insecureContentStatus, schemeIsCryptographic) {
  ...
  _onSecurityStateChanged: function(event) {
    ...
    var explanations =
        /** @type {!Array<!SecurityAgent.SecurityStateExplanation>} */ (data.explanations);
    var insecureContentStatus =
        /** @type {?SecurityAgent.InsecureContentStatus} */ (data.insecureContentStatus);
  ...
  selectAndSwitchToMainView: function() {
    // The sidebar element will trigger displaying the main view. Rather than making a redundant
    // call to display the main view, we rely on this.
  ...
  _processRequest: function(request) {
    ...
    if (request.mixedContentType === NetworkAgent.RequestMixedContentType.Blockable &&
  ...
  /**
     ...
     * @return {number}
     */
  filterRequestCount: function(filterKey) { return this._filterRequestCounts.get(filterKey) || 0; },
  ...
  targetAdded: function(target) {
    ...
    if (resourceTreeModel) {
      resourceTreeModel.addEventListener(
          WebInspector.ResourceTreeModel.Events.MainFrameNavigated, this._onMainFrameNavigated,
          this);
      resourceTreeModel.addEventListener(
          WebInspector.ResourceTreeModel.Events.InterstitialShown, this._onInterstitialShown, this);
      resourceTreeModel.addEventListener(
          WebInspector.ResourceTreeModel.Events.InterstitialHidden, this._onInterstitialHidden,
          this);
    }
  ...
  _onInterstitialHidden: function() { this._sidebarTree.toggleOriginsList(false /* hidden */); },
...
WebInspector.SecurityPanel._instance = function() {
  return /** @type {!WebInspector.SecurityPanel} */ (
...
WebInspector.SecurityPanel.createCertificateViewerButton2 = function(text, origin) {
   ...
   */
  function showCertificateViewer(e) {
    function certificateCallback(names) { InspectorFrontendHost.showCertificateViewer(names); }
...
WebInspector.SecurityPanelSidebarTree = function(mainViewElement, showOriginInPanel) {
  ...
  // This message will be removed by clearOrigins() during the first new page load after the panel
  // was opened.
  ...
  /** @type {!Map<!WebInspector.SecurityPanel.Origin,
   * !WebInspector.SecurityPanelSidebarTreeElement>} */
...
WebInspector.SecurityPanelSidebarTree.prototype = {
  ...
  addOrigin: function(origin, securityState) {
    var originElement = new WebInspector.SecurityPanelSidebarTreeElement(
        origin, this._showOriginInPanel.bind(this, origin), 'security-sidebar-tree-item',
        'security-property');
   ...
   * @param {!WebInspector.SecurityPanel.Origin} origin
   */
  setMainOrigin: function(origin) { this._mainOrigin = origin; },
  ...
  updateOrigin: function(origin, securityState) {
    var originElement = /** @type {!WebInspector.SecurityPanelSidebarTreeElement} */ (
        this._elementsByOrigin.get(origin));
    ...
    if (origin === this._mainOrigin) {
      ...
      switch (securityState) {
        case SecurityAgent.SecurityState.Secure:
          newParent =
              this._originGroups.get(WebInspector.SecurityPanelSidebarTree.OriginGroupName.Secure);
          break;
        case SecurityAgent.SecurityState.Unknown:
          newParent =
              this._originGroups.get(WebInspector.SecurityPanelSidebarTree.OriginGroupName.Unknown);
          break;
        default:
          newParent = this._originGroups.get(
              WebInspector.SecurityPanelSidebarTree.OriginGroupName.NonSecure);
          break;
      }
    }
 ...
 */
WebInspector.SecurityPanelSidebarTreeElement = function(
    text, selectCallback, className, cssPrefix) {
...
WebInspector.SecurityMainView = function(panel) {
  ...
  this._securityExplanationsMain =
      this.contentElement.createChild('div', 'security-explanation-list');
  this._securityExplanationsExtra = this.contentElement.createChild(
      'div', 'security-explanation-list security-explanations-extra');
  ...
  lockSpectrum.createChild('div', 'lock-icon lock-icon-secure').title =
      WebInspector.UIString('Secure');
  lockSpectrum.createChild('div', 'lock-icon lock-icon-neutral').title =
      WebInspector.UIString('Not Secure');
  lockSpectrum.createChild('div', 'lock-icon lock-icon-insecure').title =
      WebInspector.UIString('Insecure (Broken)');
...
WebInspector.SecurityMainView.prototype = {
  ...
  updateSecurityState: function(
    ...
    this._panel.setDisplayedInsecureContentStyle(
        insecureContentStatus.displayedInsecureContentStyle);
  ...
  refreshExplanations: function() {
    ...
    if (this._schemeIsCryptographic && this._insecureContentStatus &&
        (!this._insecureContentStatus.displayedMixedContent &&
         !this._insecureContentStatus.ranMixedContent &&
  ...
  _addMixedContentExplanations: function() {
    ...
    if (this._insecureContentStatus && (this._insecureContentStatus.ranMixedContent ||
                                        this._insecureContentStatus.displayedMixedContent)) {
      ...
      if (this._insecureContentStatus.displayedMixedContent)
        this._addMixedContentExplanation(
            this._securityExplanationsMain,
            this._insecureContentStatus.displayedInsecureContentStyle,
            WebInspector.UIString('Mixed Content'),
            WebInspector.UIString('The site includes HTTP resources.'),
            WebInspector.NetworkLogView.MixedContentFilterValues.Displayed,
            showDisplayedMixedContentInNetworkPanel);
    }

    if (this._panel.filterRequestCount(
            WebInspector.NetworkLogView.MixedContentFilterValues.Blocked) > 0)
      this._addMixedContentExplanation(
   ...
   */
  _addMixedContentExplanation: function(
      parent, securityState, summary, description, filterKey, networkFilterFn) {
    ...
    var requestsAnchor = this._addExplanation(parent, mixedContentExplanation)
                             .createChild('div', 'security-mixed-content link');
    if (filterRequestCount > 0) {
      requestsAnchor.textContent = WebInspector.UIString(
          'View %d request%s in Network Panel', filterRequestCount,
          (filterRequestCount > 1 ? 's' : ''));
    } else {
      // Network instrumentation might not have been enabled for the page load, so the security
      // panel does not necessarily know a count of individual mixed requests at this point. Point
      // the user at the Network Panel which prompts them to refresh.
...
WebInspector.SecurityOriginView = function(panel, origin, originState) {
  ...
  if (originState.securityDetails) {
    ...
    table.addRow(
        'Cipher Suite', originState.securityDetails.cipher +
    ...
    if (originState.securityDetails.signedCertificateTimestampList.length) {
      // Create the Certificate Transparency section outside the callback, so that it appears in the
      // right place.
    ...
    for (var i = 0; i < originState.securityDetails.signedCertificateTimestampList.length; i++) {
      ...
      sctTable.addRow(
          WebInspector.UIString('Signature Data'), sct.signatureData.replace(/(.{2})/g, '$1 '));
    }
    ...
    // TODO(lgarron): Fix the issue and then remove this section. See comment in
    // SecurityPanel._processRequest().
      ...
      if (listIsTruncated) {
        ...
        function toggleSANTruncation() {
          if (sanDiv.classList.contains('truncated-san')) {
            ...
            truncatedSANToggle.textContent =
                WebInspector.UIString('Show more (%d total)', sanList.length);
          }
        }
