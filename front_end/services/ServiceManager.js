...
WebInspector.ServiceManager.prototype = {
  ...
  _sendCommand: function(method, params) {
    ...
    this._connect().then(
        () => this._socket ? this._socket.send(message) : this._callbacks.get(id)(null));
...
WebInspector.ServiceManager.Service.prototype = {
   ...
   */
  on: function(methodName, handler) { this._notificationHandlers.set(methodName, handler); },
  ...
  _dispatchNotification: function(methodName, params) {
    ...
    if (!handler) {
      console.error(
          'Could not report notification \'' + methodName + '\' on \'' + this._objectId + '\'');
      return;
    }
    handler(params);
  }
