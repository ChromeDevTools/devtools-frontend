...
WebInspector.SourcesTextEditor = function(delegate) {
  ...
  this._blockIndentController =
      new WebInspector.SourcesTextEditor.BlockIndentController(this.codeMirror());
  this._tokenHighlighter =
      new WebInspector.SourcesTextEditor.TokenHighlighter(this, this.codeMirror());
   ...
   */
  function updateAnticipateJumpFlag(value) { this._isHandlingMouseDownEvent = value; }
...
WebInspector.SourcesTextEditor.prototype = {
  /**
     * @return {boolean}
     */
  _isSearchActive: function() { return !!this._tokenHighlighter.highlightedRegex(); },
    ...
    function innerHighlightRegex() {
      if (range) {
        ...
        if (range.endColumn > WebInspector.CodeMirrorTextEditor.maxHighlightLength)
          ...
          this.setSelection(
              WebInspector.TextRange.createFromLocation(range.startLine, range.startColumn));
      }
  ...
  cancelSearchResultsHighlight: function() {
    this.codeMirror().operation(
        this._tokenHighlighter.highlightSelectedTokens.bind(this._tokenHighlighter));
   ...
   */
  removeHighlight: function(highlightDescriptor) { highlightDescriptor.clear(); },
  ...
  highlightRange: function(range, cssClass) {
    ...
    return this.codeMirror().markText(
        pos.start, pos.end,
        {className: cssClass, startStyle: cssClass + '-start', endStyle: cssClass + '-end'});
  },
  ...
  setExecutionLocation: function(lineNumber, columnNumber) {
    ...
    this._executionLineTailMarker = this.codeMirror().markText(
        ...
        {line: lineNumber, ch: this.codeMirror().getLine(lineNumber).length},
  ...
  _contextMenu: function(event) {
    ...
    var promise;
    if (target) {
      promise = this._delegate.populateLineGutterContextMenu(
          contextMenu, parseInt(target.textContent, 10) - 1);
    } else {
      var textSelection = this.selection();
      promise = this._delegate.populateTextAreaContextMenu(
          contextMenu, textSelection.startLine, textSelection.startColumn);
    }
  ...
  editRange: function(range, text, origin) {
    var newRange =
        WebInspector.CodeMirrorTextEditor.prototype.editRange.call(this, range, text, origin);
  ...
  /**
     * @return {string}
     */
  indent: function() { return this._indentationLevel; },
  ...
  _onAutoAppendedSpaces: function() {
    ...
    for (var i = 0; i < this._autoAppendedSpaces.length; ++i) {
      ...
      if (line.length === position.columnNumber &&
    ...
    for (var i = 0; i < selections.length; ++i) {
      ...
      this._autoAppendedSpaces.push(
          this.textEditorPositionHandle(selection.startLine, selection.startColumn));
    }
  },
  ...
  _scroll: function() {
    var topmostLineNumber =
        this.codeMirror().lineAtHeight(this.codeMirror().getScrollInfo().top, 'local');
    this._delegate.scrollChanged(topmostLineNumber);
  },

  _focus: function() { this._delegate.editorFocused(); },

  _blur: function() { this._delegate.editorBlurred(); },
  ...
  _beforeSelectionChangeForDelegate: function(codeMirror, selection) {
    ...
    this._reportJump(
        this.selection(),
        WebInspector.CodeMirrorUtils.toRange(primarySelection.anchor, primarySelection.head));
  },
  ...
  setText: function(text) {
    ...
    this._setEditorIndentation(text.split('\n').slice(
        0, WebInspector.SourcesTextEditor.LinesToScanForIndentationGuessing));
  ...
  setMimeType: function(mimeType) {
    ...
    WebInspector.CodeMirrorTextEditor.prototype.setMimeType.call(
        this, this._applyWhitespaceMimetype(mimeType));
  },
  ...
  _setupWhitespaceHighlight: function() {
    ...
    if (doc._codeMirrorWhitespaceStyleInjected ||
        !WebInspector.moduleSetting('showWhitespacesInEditor').get())
      return;
    ...
    var spaceChars = '';
    var rules = '';
    for (var i = 1; i <= WebInspector.SourcesTextEditor.MaximumNumberOfWhitespacesPerSingleSpan;
         ++i) {
...
CodeMirror.commands.smartNewlineAndIndent = function(codeMirror) {
  ...
  function innerSmartNewlineAndIndent(codeMirror) {
    ...
    for (var i = 0; i < selections.length; ++i) {
      var selection = selections[i];
      var cur = CodeMirror.cmpPos(selection.head, selection.anchor) < 0 ? selection.head :
                                                                          selection.anchor;
...
CodeMirror.commands.sourcesDismiss = function(codemirror) {
  if (codemirror.listSelections().length === 1 &&
      codemirror._codeMirrorTextEditor._isSearchActive())
    return CodeMirror.Pass;
...
WebInspector.SourcesTextEditor.BlockIndentController.prototype = {
  ...
  Enter: function(codeMirror) {
    ...
    for (var i = 0; i < selections.length; ++i) {
      var selection = selections[i];
      var start = CodeMirror.cmpPos(selection.head, selection.anchor) < 0 ? selection.head :
                                                                            selection.anchor;
  ...
  '\'}\'': function(codeMirror) {
    ...
    for (var i = 0; i < selections.length; ++i) {
      ...
      updatedSelections.push(
          {head: selection.head, anchor: new CodeMirror.Pos(selection.head.line, 0)});
...
WebInspector.SourcesTextEditor.TokenHighlighter.prototype = {
  ...
  highlightSearchResults: function(regex, range) {
    ...
    if (this._highlightDescriptor && this._highlightDescriptor.selectionStart)
      this._codeMirror.removeLineClass(
          this._highlightDescriptor.selectionStart.line, 'wrap', 'cm-line-with-selection');
    ...
    if (this._highlightRegex === oldRegex) {
      ...
      this._setHighlighter(
          this._searchHighlighter.bind(this, this._highlightRegex), selectionStart);
    }
    if (this._highlightRange) {
      var pos = WebInspector.CodeMirrorUtils.toPos(this._highlightRange);
      this._searchResultMarker =
          this._codeMirror.markText(pos.start, pos.end, {className: 'cm-column-with-selection'});
    }
  },
  ...
  /**
     ...
     */
  highlightedRegex: function() { return this._highlightRegex; },
  ...
  highlightSelectedTokens: function() {
    ...
    if (this._isWord(selectedText, selectionStart.line, selectionStart.ch, selectionEnd.ch)) {
      if (selectionStart)
        this._codeMirror.addLineClass(selectionStart.line, 'wrap', 'cm-line-with-selection');
      this._setHighlighter(
          this._tokenHighlighter.bind(this, selectedText, selectionStart), selectionStart);
    }
  },
  ...
  _isWord: function(selectedText, lineNumber, startColumn, endColumn) {
    ...
    var leftBound =
        startColumn === 0 || !WebInspector.TextUtils.isWordChar(line.charAt(startColumn - 1));
    var rightBound =
        endColumn === line.length || !WebInspector.TextUtils.isWordChar(line.charAt(endColumn));
  ...
  _tokenHighlighter: function(token, selectionStart, stream) {
    ...
    if (stream.match(token) && (stream.eol() || !WebInspector.TextUtils.isWordChar(stream.peek())))
      return stream.column() === selectionStart.ch ? 'token-highlight column-with-selection' :
                                                     'token-highlight';
    var eatenChar;
    do {
      eatenChar = stream.next();
    } while (eatenChar &&
             (WebInspector.TextUtils.isWordChar(eatenChar) || stream.peek() !== tokenFirstChar));
  },
