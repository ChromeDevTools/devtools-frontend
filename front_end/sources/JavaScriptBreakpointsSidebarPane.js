...
WebInspector.JavaScriptBreakpointsSidebarPane.prototype = {
  ...
  _appendBreakpointActiveItem: function(contextMenu) {
    ...
    contextMenu.appendItem(
        breakpointActiveTitle, this._breakpointManager.setBreakpointsActive.bind(
                                   this._breakpointManager, !breakpointActive));
  },
  ...
  _addBreakpoint: function(breakpoint, uiLocation) {
    ...
    element.addEventListener(
        'contextmenu', this._breakpointContextMenu.bind(this, breakpoint), true);
    ...
    checkboxLabel.addEventListener(
        'click', this._breakpointCheckboxClicked.bind(this, breakpoint), false);
   ...
   * @param {?Object} object
   */
  flavorChanged: function(object) { this._update(); },
  ...
  _update: function() {
    ...
    var uiLocation = details && details.callFrames.length ?
        WebInspector.debuggerWorkspaceBinding.rawLocationToUILocation(
            details.callFrames[0].location()) :
        null;
    var breakpoint = uiLocation ?
        this._breakpointManager.findBreakpointOnLine(
  ...
  _breakpointsActiveStateChanged: function() {
    this._listElement.classList.toggle(
        'breakpoints-list-deactivated', !this._breakpointManager.breakpointsActive());
  },

  /**
   * @param {!WebInspector.UILocation} uiLocation
   */
  _breakpointClicked: function(uiLocation) { WebInspector.Revealer.reveal(uiLocation); },
  ...
  _breakpointContextMenu: function(breakpoint, event) {
    ...
    var contextMenu = new WebInspector.ContextMenu(event);
    contextMenu.appendItem(
        WebInspector.UIString.capitalize('Remove ^breakpoint'), breakpoint.remove.bind(breakpoint));
    if (breakpoints.length > 1) {
      var removeAllTitle = WebInspector.UIString.capitalize('Remove ^all ^breakpoints');
      contextMenu.appendItem(
          removeAllTitle,
          this._breakpointManager.removeAllBreakpoints.bind(this._breakpointManager));
    }
    ...
    if (breakpoints.length > 1) {
      ...
      contextMenu.appendItem(
          enableTitle,
          this._breakpointManager.toggleAllBreakpoints.bind(this._breakpointManager, true),
          !(enableBreakpointCount !== breakpoints.length));
      contextMenu.appendItem(
          disableTitle,
          this._breakpointManager.toggleAllBreakpoints.bind(this._breakpointManager, false),
  ...
  _compareBreakpoints: function(b1, b2) {
    return this._compare(b1.uiSourceCode.url(), b2.uiSourceCode.url()) ||
