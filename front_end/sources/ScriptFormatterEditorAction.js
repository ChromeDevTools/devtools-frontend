    ...
    for (var i = 0; i < formatData.scripts.length; ++i) {
      if (formatData.scripts[i].debuggerModel === this._debuggerModel)
        return this._debuggerModel.createRawLocation(
            formatData.scripts[i], originalLocation[0], originalLocation[1]);
    }
    return null;
  },
...
WebInspector.ScriptFormatterEditorAction.prototype = {
  ...
  targetAdded: function(target) {
    ...
    this._scriptMappingByTarget.set(
        target, new WebInspector.FormatterScriptMapping(debuggerModel, this));
  ...
  button: function(sourcesView) {
    ...
    this._button = new WebInspector.ToolbarButton(
        WebInspector.UIString('Pretty print'), 'format-toolbar-item');
  ...
  _isFormatableScript: function(uiSourceCode) {
    ...
    var supportedProjectTypes = [
      WebInspector.projectTypes.Network, WebInspector.projectTypes.Debugger,
  ...
  _scriptsForUISourceCode: function(uiSourceCode) {
    /**
         ...
         * @return {boolean}
         */
    function isInlineScript(script) { return script.isInlineScript() && !script.hasSourceURL; }
    ...
    if (uiSourceCode.contentType().isScript()) {
      var rawLocations =
          WebInspector.debuggerWorkspaceBinding.uiLocationToRawLocations(uiSourceCode, 0, 0);
  ...
  _formatUISourceCodeScript: function(uiSourceCode) {
    var formattedPath =
        this._formattedPaths.get(uiSourceCode.project().id() + ':' + uiSourceCode.url());
    if (formattedPath) {
      ...
      if (formatData)
        this._showIfNeeded(
            uiSourceCode, /** @type {!WebInspector.UISourceCode} */ (formattedUISourceCode),
            formatData.mapping);
      return;
    }
    ...
    function innerCallback(formattedContent, formatterMapping) {
      ...
      var contentProvider = WebInspector.StaticContentProvider.fromString(
      ...
      for (var i = 0; i < scripts.length; ++i) {
        ...
        var scriptMapping = /** @type {!WebInspector.FormatterScriptMapping} */ (
            this._scriptMappingByTarget.get(scripts[i].target()));
      ...
      for (var i = 0; i < targets.length; ++i) {
        var scriptMapping = /** @type {!WebInspector.FormatterScriptMapping} */ (
            this._scriptMappingByTarget.get(targets[i]));
        WebInspector.debuggerWorkspaceBinding.setSourceMapping(
            targets[i], formattedUISourceCode, scriptMapping);
      }
