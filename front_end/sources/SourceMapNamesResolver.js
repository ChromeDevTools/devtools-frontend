...
WebInspector.SourceMapNamesResolver._scopeIdentifiers = function(scope) {
  ...
  if (scope.type() === DebuggerAgent.ScopeType.Global || !startLocation || !endLocation ||
    ...
    return Promise.resolve(
        /** @type {!Array<!WebInspector.SourceMapNamesResolver.Identifier>}*/ ([]));
  ...
  function onContent(content) {
    ...
    var scopeRange = new WebInspector.TextRange(
        startLocation.lineNumber, startLocation.columnNumber, endLocation.lineNumber,
    ...
    return WebInspector.formatterWorkerPool
        .runTask('javaScriptIdentifiers', {content: prefix + scopeText})
  ...
  function onIdentifiers(text, scopeStart, prefix, event) {
    var identifiers =
        event ? /** @type {!Array<!{name: string, offset: number}>} */ (event.data) : [];
    var result = [];
    ...
    for (var i = 0; i < identifiers.length; ++i) {
      ...
      result.push(new WebInspector.SourceMapNamesResolver.Identifier(
          id.name, cursor.lineNumber(), cursor.columnNumber()));
    }
    return result;
  }
};
...
WebInspector.SourceMapNamesResolver._resolveScope = function(scope) {
  ...
  identifiersPromise =
      WebInspector.SourceMapNamesResolver._scopeIdentifiers(scope).then(onIdentifiers);
  ...
  function resolveSourceName(id) {
    ...
    if (!startEntry || !endEntry || !startEntry.sourceURL ||
        startEntry.sourceURL !== endEntry.sourceURL || !startEntry.sourceLineNumber ||
        !startEntry.sourceColumnNumber || !endEntry.sourceLineNumber ||
        !endEntry.sourceColumnNumber)
      return Promise.resolve(/** @type {?string} */ (null));
    var sourceTextRange = new WebInspector.TextRange(
        startEntry.sourceLineNumber, startEntry.sourceColumnNumber, endEntry.sourceLineNumber,
        endEntry.sourceColumnNumber);
    var uiSourceCode =
        WebInspector.networkMapping.uiSourceCodeForScriptURL(startEntry.sourceURL, script);
    if (!uiSourceCode)
      return Promise.resolve(/** @type {?string} */ (null));
 ...
 * @return {!Promise<string>}
 */
WebInspector.SourceMapNamesResolver.resolveExpression = function(
  ...
  return WebInspector.SourceMapNamesResolver._allVariablesInCallFrame(callFrame).then(
      findCompiledName);
  ...
  function findCompiledName(reverseMapping) {
    ...
    return WebInspector.SourceMapNamesResolver._resolveExpression(
 ...
 * @return {!Promise<string>}
 */
WebInspector.SourceMapNamesResolver._resolveExpression = function(
    callFrame, uiSourceCode, lineNumber, startColumnNumber, endColumnNumber) {
  ...
  function onContent(content) {
    ...
    var textRange = sourceMap.reverseMapTextRange(
        uiSourceCode.url(),
        new WebInspector.TextRange(lineNumber, startColumnNumber, lineNumber, endColumnNumber));
    var originalText = text.extract(textRange);
    if (!originalText)
      return Promise.resolve('');
    return WebInspector.formatterWorkerPool
        .runTask('evaluatableJavaScriptSubstring', {content: originalText})
        .then(onResult);
  }
  ...
  /**
     ...
     * @return {string}
     */
  function onResult(event) { return event ? /** @type {string} */ (event.data) : ''; }
};
...
WebInspector.SourceMapNamesResolver.resolveThisObject = function(callFrame) {
  ...
  function onScopeResolved(namesMapping) {
    ...
    callFrame.evaluate(
        thisMapping, 'backtrace', false, true, false, true, onEvaluated.bind(null, callback));
    return promise;
  }
  ...
  function onEvaluated(callback, evaluateResult) {
    var remoteObject = evaluateResult ?
        callFrame.target().runtimeModel.createRemoteObject(evaluateResult) :
...
WebInspector.SourceMapNamesResolver.RemoteObject.prototype = {
  /**
     ...
     */
  customPreview: function() { return this._object.customPreview(); },
  ...
  /**
     ...
     * @return {number}
     */
  arrayLength: function() { return this._object.arrayLength(); },
   ...
   */
  getOwnProperties: function(callback) { this._object.getOwnProperties(callback); },
  ...
  getAllProperties: function(accessorPropertiesOnly, callback) {
    ...
    function wrappedCallback(properties, internalProperties) {
      WebInspector.SourceMapNamesResolver._resolveScope(this._scope)
    ...
    function resolveNames(properties, internalProperties, namesMapping) {
      ...
      if (properties) {
        for (var i = 0; i < properties.length; ++i) {
          ...
          newProperties.push(new WebInspector.RemoteObjectProperty(
              name, property.value, property.enumerable, property.writable, property.isOwn,
  ...
  /**
     ...
     */
  eventListeners: function() { return this._object.eventListeners(); },
   ...
   * @param {function(string=)} callback
   */
  deleteProperty: function(name, callback) { this._object.deleteProperty(name, callback); },
  ...
  /**
     ...
     */
  target: function() { return this._object.target(); },

  /**
     * @override
     * @return {?WebInspector.DebuggerModel}
     */
  debuggerModel: function() { return this._object.debuggerModel(); },

  /**
     * @override
     * @return {boolean}
     */
  isNode: function() { return this._object.isNode(); },
