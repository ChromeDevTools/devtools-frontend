...
WebInspector.WorkspaceMappingTip = function(sourcesPanel, workspace) {
  ...
  this._workspaceInfobarDisabledSetting =
      WebInspector.settings.createSetting('workspaceInfobarDisabled', false);
  ...
  if (this._workspaceInfobarDisabledSetting.get() &&
      this._workspaceMappingInfobarDisabledSetting.get())
    return;
...
WebInspector.WorkspaceMappingTip.prototype = {
  ...
  _editorSelected: function(event) {
    ...
    this._editorSelectedTimer =
        setTimeout(this._updateSuggestedMappingInfobar.bind(this, uiSourceCode), 3000);
  },
  ...
  _updateSuggestedMappingInfobar: function(uiSourceCode) {
    ...
    if (!this._workspaceMappingInfobarDisabledSetting.get() &&
      ...
      networkProjects = networkProjects.concat(
          this._workspace.projectsForType(WebInspector.projectTypes.ContentScripts));
      for (var i = 0; i < networkProjects.length; ++i) {
        ...
        for (var j = 0; j < networkUiSourceCodes.length; ++j) {
          if (networkUiSourceCodes[j].name() === name &&
              this._isLocalHost(networkUiSourceCodes[j].url())) {
    ...
    if (uiSourceCode.project().type() === WebInspector.projectTypes.Network ||
      ...
      var filesystemProjects =
          this._workspace.projectsForType(WebInspector.projectTypes.FileSystem);
  ...
  _showWorkspaceInfobar: function(uiSourceCode) {
    ...
    infobar.createDetailsRowMessage(WebInspector.UIString(
        'To add a folder into the workspace, drag and drop it into the Sources panel.'));
  ...
  _showMappingInfobar: function(uiSourceCode, isNetwork) {
    ...
    if (isNetwork)
      title =
          WebInspector.UIString('Map network resource \'%s\' to workspace?', uiSourceCode.url());
    else
      title =
          WebInspector.UIString('Map workspace resource \'%s\' to network?', uiSourceCode.url());
    ...
    infobar.createDetailsRowMessage(
        WebInspector.UIString('Use context menu to establish the mapping at any time.'));
  ...
  _appendInfobar: function(uiSourceCode, infobar) {
    ...
    var rowElement = infobar.createDetailsRowMessage(
        WebInspector.UIString('For more information on workspaces, refer to the '));
