...
export function setupUserMetricHooks(): void {
  beforeEach(() => {
    const mockEnumCodes = new Map<string, Map<string, number>>();
    function getMockEnumCode(enumName: string, prop: string): number {
      if (prop === 'MAX_VALUE') {
        return 1000;
      }
    ...
    (globalThis as unknown as {DevToolsMetrics: unknown}).DevToolsMetrics = new Proxy({}, {
  ...
  afterEach(() => {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    const globalObject = globalThis as unknown as {DevToolsMetrics?: unknown};
