...
WebInspector.TextEditorAutocompleteController.prototype = {
  ...
  _wordsWithPrefix: function(prefixRange, substituteRange) {
    ...
    completions.sort(
        (a, b) =>
            this._dictionary.wordCount(b) - this._dictionary.wordCount(a) || a.length - b.length);
  ...
  _changes: function(codeMirror, changes) {
    ...
    for (var changeIndex = 0; changeIndex < changes.length; ++changeIndex) {
      ...
      if (changeObject.origin === '+input' && changeObject.text.length === 1 &&
          changeObject.text[0].length === 1 && changeObject.to.line === cursor.line &&
          changeObject.to.ch + 1 === cursor.ch) {
        singleCharInput = true;
        break;
      }
      if (this._suggestBox && changeObject.origin === '+delete' &&
          changeObject.removed.length === 1 && changeObject.removed[0].length === 1 &&
    ...
    if (singleCharDelete && this._hintMarker && this._lastPrefix) {
      this._hintElement.textContent =
          this._lastPrefix.charAt(this._lastPrefix.length - 1) + this._hintElement.textContent;
  ...
  autocomplete: function() {
    ...
    function wordsAcquired(wordsWithPrefix) {
      if (!wordsWithPrefix.length ||
          (wordsWithPrefix.length === 1 && prefix === wordsWithPrefix[0].title) ||
      ...
      if (!oldPrefixRange || prefixRange.startLine !== oldPrefixRange.startLine ||
          prefixRange.startColumn !== oldPrefixRange.startColumn)
        this._updateAnchorBox();
      this._suggestBox.updateSuggestions(
          this._anchorBox, wordsWithPrefix, 0, !this._isCursorAtEndOfLine(), prefix);
  ...
  _onCursorActivity: function() {
    ...
    if (cursor.line !== this._prefixRange.startLine || cursor.ch > this._prefixRange.endColumn ||
