...
WebInspector.TimelineController.prototype = {
  ...
  startRecording: function(
      captureCauses, enableJSSampling, captureMemory, capturePictures, captureFilmStrip) {
    this._extensionTraceProviders = WebInspector.extensionServer.traceProviders().slice();

    function disabledByDefault(category) { return 'disabled-by-default-' + category; }
    var categoriesArray = [
  ...
  _stopProfilingOnTarget: function(target) {
    return target.hasJSCapability() ?
        target.profilerAgent().stop(this._addCpuProfile.bind(this, target.id())) :
        Promise.resolve();
  },
  ...
  _addCpuProfile: function(targetId, error, cpuProfile) {
    if (!cpuProfile) {
      WebInspector.console.warn(
          WebInspector.UIString('CPU profile for a target is not available. %s', error || ''));
      return;
    }
  ...
  _startRecordingWithCategories: function(categories, enableJSSampling, callback) {
    ...
    var profilingStartedPromise =
        enableJSSampling && !Runtime.experiments.isEnabled('timelineTracingJSProfile') ?
        this._startProfilingOnAllTargets() :
        Promise.resolve();
    var samplingFrequencyHz =
        WebInspector.moduleSetting('highResolutionCpuProfiling').get() ? 10000 : 1000;
    ...
    profilingStartedPromise.then(
        tracingManager.start.bind(tracingManager, this, categories, options, onTraceStarted));
    /**
     * @param {?string} error
     */
   ...
   * @override
   */
  traceEventsCollected: function(events) { this._tracingModel.addEvents(events); },
  ...
  _injectCpuProfileEvents: function() {
    ...
    var mainMetaEvent =
        metadataEvents.filter(event => event.name === metadataEventTypes.TracingStartedInPage)
            .peekLast();
    if (!mainMetaEvent)
      return;
    ...
    for (var metaEvent of workerMetaEvents) {
      ...
      var workerTarget =
          this._target.workerManager ? this._target.workerManager.targetByWorkerId(workerId) : null;
      if (!workerTarget)
        continue;
   ...
   * @override
   */
  tracingBufferUsage: function(usage) { this._delegate.recordingProgress(usage); },

  /**
   * @param {number} progress
   * @override
   */
  eventsRetrievalProgress: function(progress) { this._delegate.loadingProgress(progress); }
};
