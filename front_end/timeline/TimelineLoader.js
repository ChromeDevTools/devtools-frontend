...
WebInspector.TimelineLoader = function(model, delegate) {
  ...
  this._totalSize;
  this._jsonTokenizer =
      new WebInspector.TextUtils.BalancedJSONTokenizer(this._writeBalancedJSON.bind(this), true);
};
...
WebInspector.TimelineLoader._createFileReader = function(file, delegate) {
  return new WebInspector.ChunkedFileReader(
      file, WebInspector.TimelineLoader.TransferChunkLengthBytes, delegate);
};
...
WebInspector.TimelineLoader.prototype = {
  ...
  write: function(chunk) {
    ...
    if (!this._firstChunk)
      this._delegate.loadingProgress(
          this._totalSize ? this._loadedBytes / this._totalSize : undefined);
    ...
    if (this._state === WebInspector.TimelineLoader.State.Initial) {
      if (chunk[0] === '{')
        ...
        this._reportErrorAndCancelLoading(
            WebInspector.UIString('Malformed timeline data: Unknown JSON format'));
        return;
      }
    }
    ...
    if (this._firstChunk) {
      this._reportErrorAndCancelLoading(
          WebInspector.UIString('Malformed timeline input, wrong JSON brackets balance'));
      return;
    }
  },
  ...
  _writeBalancedJSON: function(data) {
    ...
    try {
      ...
      this._reportErrorAndCancelLoading(
          WebInspector.UIString('Malformed timeline data: %s', e.toString()));
      return;
    }
    ...
    if (this._firstChunk) {
      ...
      if (this._looksLikeAppVersion(items[0])) {
        this._reportErrorAndCancelLoading(
            WebInspector.UIString('Legacy Timeline format is not supported.'));
        return;
      }
    }
  ...
  onError: function(reader, event) {
    switch (event.target.error.name) {
      case 'NotFoundError':
        this._reportErrorAndCancelLoading(
            WebInspector.UIString('File "%s" not found.', reader.fileName()));
        break;
      case 'NotReadableError':
        this._reportErrorAndCancelLoading(
            WebInspector.UIString('File "%s" is not readable', reader.fileName()));
        break;
      case 'AbortError':
        break;
      default:
        this._reportErrorAndCancelLoading(WebInspector.UIString(
            'An error occurred while reading the file "%s"', reader.fileName()));
    }
  }
};
...
WebInspector.TracingTimelineSaver.prototype = {
  ...
  onError: function(reader, event) {
    ...
    WebInspector.console.error(WebInspector.UIString(
        'Failed to save timeline: %s (%s, %s)', error.message, error.name, error.code));
  }
};
