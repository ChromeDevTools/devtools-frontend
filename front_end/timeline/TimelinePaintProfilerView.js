...
WebInspector.TimelinePaintProfilerView = function(frameModel) {
  ...
  this._paintProfilerView =
      new WebInspector.PaintProfilerView(this._imageView.showImage.bind(this._imageView));
...
WebInspector.TimelinePaintProfilerView.prototype = {
  ...
  _update: function() {
    ...
    function onDataAvailable(data) {
      if (data)
        WebInspector.PaintProfilerSnapshot.load(
            this._target, data['skp64'], onSnapshotLoaded.bind(this, null));
    }
    /**
  ...
  _updateImagePosition: function() {
    ...
    if (this._maskRectangle) {
      ...
      style.borderBottomWidth =
          (height - this._maskRectangle.y - this._maskRectangle.height) + 'px';
    }
    ...
    var bounds =
        WebInspector.Geometry.boundsForTransformedPoints(matrix, [0, 0, 0, width, height, 0]);
