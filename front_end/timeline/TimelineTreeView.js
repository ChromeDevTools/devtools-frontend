...
WebInspector.TimelineTreeView = function(model, filters) {
  ...
  this._dataGrid.addEventListener(
      WebInspector.DataGrid.Events.SortingChanged, this._sortingChanged, this);
  ...
  this._splitWidget =
      new WebInspector.SplitWidget(true, true, 'timelineTreeViewDetailsSplitWidget');
  ...
  this._dataGrid.addEventListener(
      WebInspector.DataGrid.Events.SelectedNode, this._updateDetailsForSelection, this);
...
WebInspector.TimelineTreeView.prototype = {
  ...
  /**
     * @return {boolean}
     */
  _exposePercentages: function() { return false; },
  ...
  _refreshTree: function() {
    ...
    for (var child of tree.children.values()) {
      ...
      var gridNode = new WebInspector.TimelineTreeView.TreeGridNode(
          child, tree.totalTime, maxSelfTime, maxTotalTime, this);
  ...
  /**
     ...
     */
  _buildTree: function() { throw new Error('Not Implemented'); },
  ...
  _populateColumns: function(columns) {
    ...
    columns.push({
      id: 'activity',
      title: WebInspector.UIString('Activity'),
      disclosure: true,
      sortable: true
    });
  },
  ...
  _updateDetailsForSelection: function() {
    var selectedNode = this._dataGrid.selectedNode ?
        /** @type {!WebInspector.TimelineTreeView.TreeGridNode} */ (this._dataGrid.selectedNode)
            ._profileNode :
        null;
  ...
  /**
     ...
     * @return {boolean}
     */
  _showDetailsForNode: function(node) { return false; },
 ...
 */
WebInspector.TimelineTreeView.GridNode = function(
    profileNode, grandTotalTime, maxSelfTime, maxTotalTime, treeView) {
...
WebInspector.TimelineTreeView.GridNode.prototype = {
  ...
  _createValueCell: function(columnIdentifier) {
    ...
    if (maxTime) {
      ...
      cell.createChild('div', 'background-bar-container')
          .createChild('div', 'background-bar')
          .style.width = (value * 100 / maxTime).toFixed(1) + '%';
    }
    return cell;
  },
 ...
 * @param {!WebInspector.TimelineTreeView} treeView
 */
WebInspector.TimelineTreeView.TreeGridNode = function(
    profileNode, grandTotalTime, maxSelfTime, maxTotalTime, treeView) {
  WebInspector.TimelineTreeView.GridNode.call(
      this, profileNode, grandTotalTime, maxSelfTime, maxTotalTime, treeView);
...
WebInspector.AggregatedTimelineTreeView = function(model, filters) {
  this._groupBySetting = WebInspector.settings.createSetting(
      'timelineTreeGroupBy', WebInspector.TimelineAggregator.GroupBy.Category);
  ...
  this._stackView.addEventListener(
      WebInspector.TimelineStackView.Events.SelectionChanged, this._onStackViewSelectionChanged,
      this);
};
...
WebInspector.AggregatedTimelineTreeView.prototype = {
  ...
  _displayInfoForGroupNode: function(node) {
    ...
    var color =
        node.id ? WebInspector.TimelineUIUtils.eventColor(node.event) : categories['other'].color;
  ...
  _populateToolbar: function(parent) {
    ...
    addGroupingOption.call(
        this, WebInspector.UIString('No Grouping'), WebInspector.TimelineAggregator.GroupBy.None);
    addGroupingOption.call(
        this, WebInspector.UIString('Group by Activity'),
        WebInspector.TimelineAggregator.GroupBy.EventName);
    addGroupingOption.call(
        this, WebInspector.UIString('Group by Category'),
        WebInspector.TimelineAggregator.GroupBy.Category);
    addGroupingOption.call(
        this, WebInspector.UIString('Group by Domain'),
        WebInspector.TimelineAggregator.GroupBy.Domain);
    addGroupingOption.call(
        this, WebInspector.UIString('Group by Subdomain'),
        WebInspector.TimelineAggregator.GroupBy.Subdomain);
    addGroupingOption.call(
        this, WebInspector.UIString('Group by URL'), WebInspector.TimelineAggregator.GroupBy.URL);
  ...
  /**
     ...
     * @return {boolean}
     */
  _exposePercentages: function() { return true; },
...
WebInspector.EventsTimelineTreeView.prototype = {
  ...
  _showDetailsForNode: function(node) {
     ...
     */
    function showDetails(fragment) { this._detailsView.element.appendChild(fragment); }
  },
   ...
   * @param {?WebInspector.TimelineProfileTree.Node} node
   */
  _onHover: function(node) { this._delegate.highlightEvent(node && node.event); },
...
WebInspector.TimelineStackView = function(treeView) {
  ...
  this._dataGrid.setResizeMethod(WebInspector.DataGrid.ResizeMethod.Last);
  this._dataGrid.addEventListener(
      WebInspector.DataGrid.Events.SelectedNode, this._onSelectionChanged, this);
...
WebInspector.TimelineStackView.prototype = {
  ...
  setStack: function(stack, selectedNode) {
    ...
    for (var node of stack) {
      var gridNode = new WebInspector.TimelineTreeView.GridNode(
          node, totalTime, totalTime, totalTime, this._treeView);
  ...
  selectedTreeNode: function() {
    var selectedNode = this._dataGrid.selectedNode;
    return selectedNode &&
        /** @type {!WebInspector.TimelineTreeView.GridNode} */ (selectedNode)._profileNode;
  },
