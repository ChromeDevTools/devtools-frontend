...
WebInspector.TimelineIRModel.prototype = {
  ...
  populate: function(inputLatencies, animations) {
    ...
    range.appendRange(this._drags);  // Drags take lower precedence than animation, as we can't
    ...
    for (var i = 0; i < events.length; ++i) {
      ...
      switch (type) {
        ...
        case eventTypes.TouchStart:
          // We do not produce any response segment for TouchStart -- there's either going to be one
          // upon
        ...
        case eventTypes.MouseMove:
          if (mouseDown && !mouseMove &&
              mouseDown.startTime + thresholdsMs.mouse > event.startTime) {
        ...
        case eventTypes.MouseWheel:
          // Do not consider first MouseWheel as trace viewer's implementation does -- in case of
          // MouseWheel it's not really special.
  ...
  _processAnimations: function(events) {
    for (var i = 0; i < events.length; ++i)
      this._cssAnimations.append(
          this._segmentForEvent(events[i], WebInspector.TimelineIRModel.Phases.Animation));
  },
  ...
  /**
     ...
     */
  interactionRecords: function() { return this._segments; },
  ...
  _inputEventType: function(eventName) {
    ...
    return /** @type {!WebInspector.TimelineIRModel.InputEvents} */ (
        eventName.substr(prefix.length));
  }
};
