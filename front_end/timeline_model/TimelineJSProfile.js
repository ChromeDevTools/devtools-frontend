 ...
 */
WebInspector.TimelineJSProfileProcessor.generateTracingEventsFromCpuProfile = function(
  ...
  for (var i = 0; i < samples.length; ++i) {
    ...
    var jsSampleEvent = new WebInspector.TracingModel.Event(
        ...
        WebInspector.TimelineModel.RecordType.JSSample, WebInspector.TracingModel.Phase.Instant,
        timestamps[i], thread);
...
WebInspector.TimelineJSProfileProcessor.generateJSFrameEvents = function(events) {
  ...
  /**
     ...
     * @return {number}
     */
  function eventEndTime(e) { return e.endTime || e.startTime; }
  ...
  function truncateJSStack(depth, time) {
    if (lockedJsStackDepth.length) {
      ...
      if (depth < lockedDepth) {
        console.error(
            'Child stack is shallower (' + depth + ') than the parent stack (' + lockedDepth +
            ') at ' + time);
        depth = lockedDepth;
      }
    }
  ...
  function extractStackTrace(e) {
    ...
    if (e.name === recordTypes.JSSample) {
      ...
      callFrames =
          /** @type {!Array<!RuntimeAgent.CallFrame>} */ (eventData && eventData['stackTrace']);
    } else {
      callFrames = /** @type {!Array<!RuntimeAgent.CallFrame>} */ (
...
WebInspector.TimelineJSProfileProcessor.CodeMap.prototype = {
  ...
  addEntry: function(addressHex, size, callFrame) {
    var entry = new WebInspector.TimelineJSProfileProcessor.CodeMap.Entry(
        this._getAddress(addressHex), size, callFrame);
...
WebInspector.TimelineJSProfileProcessor.CodeMap.Bank.prototype = {
  ...
  removeEntry: function(address) {
    var index = this._entries.lowerBound(
        address, WebInspector.TimelineJSProfileProcessor.CodeMap.comparator);
  ...
  lookupEntry: function(address) {
    var index = this._entries.upperBound(
                    address, WebInspector.TimelineJSProfileProcessor.CodeMap.comparator) -
        1;
    var entry = this._entries[index];
  ...
  addEntry: function(newEntry) {
    ...
    var lastIndex = this._entries.lowerBound(
        endAddress, WebInspector.TimelineJSProfileProcessor.CodeMap.comparator);
    var index;
  ...
  for (var i = 0; i < events.length; ++i) {
    ...
    switch (e.name) {
      case recordTypes.JitCodeAdded:
        var frame = WebInspector.TimelineJSProfileProcessor._buildCallFrame(
