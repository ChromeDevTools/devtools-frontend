...
A context is defined as set of 'flavors' or classes, which usually represent views, that are added and removed to the context as the user interacts with the application (e.g when the user moves across views).
...
The `bindings` property defines `platform` and `keybindSets` properties along with the `shortcut` property containing the actual keyboard shortcut.
...
- `platform` refers to the OS on which DevTools is running and can be either `MAC`, `WINDOWS_LINUX` or `WINDOWS`. (See [`UI.ActionRegistration`](./ActionRegistration.ts)).
...
These two key bindings both allocate the same key for the `WINDOWS_LINUX` platform (the first one is global across platforms) in the `ElementsPanel` context (the second one is global across context types) and both are global across keybind sets.
...
UI.ActionRegistration.registerActionExtension({
  ...
  bindings: [
    {
      shortcut: 'Esc',
      keybindSets: [UI.ActionRegistration.KeybindSet.DEVTOOLS_DEFAULT],
    },
  ],
});
```
...
In this case, the keybindings are declared for different keybind sets and thus do not result in duplication.
