...
var ImagePreview = class {
  static async build(originalImageURL, showDimensions, options = { align: "center" /* CENTER */ }) {
    ...
    if (!isImageResource(resource) && precomputedFeatures?.currentSrc) {
      ...
      resource = SDK.ResourceTreeModel.ResourceTreeModel.resourceForURL(SDK.TargetManager.TargetManager.instance(), imageURL);
    }
...
var Linkifier = class _Linkifier extends Common2.ObjectWrapper.ObjectWrapper {
  ...
  static linkifyUILocation(uiLocation, options) {
    ...
    const fallbackAnchor = _Linkifier.linkifyURL(uiLocation.uiSourceCode.url(), linkifyURLOptions);
    const isIgnoreListed = (options?.ignoreListManager ?? Workspace.IgnoreListManager.IgnoreListManager.instance()).isUserIgnoreListedURL(uiLocation.uiSourceCode.url());
  ...
  static linkifyStackTraceFrame(frame, options) {
    ...
    return _Linkifier.linkifyUILocation(uiLocation, options);
  }
  ...
  static renderLinkifiedUrl(url, options) {
    ...
    const isPrivileged = Common2.ParsedURL.isPrivilegedScheme(url);
...
var LinkHandlerSettingUI = class _LinkHandlerSettingUI {
  ...
  update() {
    ...
    origins.unshift(i18nString2(UIStrings2.auto));
    for (const origin of origins) {
      const option = document.createElement("option");
      const registration = linkHandlers.get(origin);
      option.textContent = registration === void 0 ? origin : registration.title;
      option.value = origin;
