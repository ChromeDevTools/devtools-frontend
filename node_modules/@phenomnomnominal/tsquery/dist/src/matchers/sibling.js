...
function sibling(node, selector, ancestry) {
    return (0, match_1.findMatches)(node, selector.right, ancestry) &&
        ...
        selector.left.subject &&
            (0, match_1.findMatches)(node, selector.left, ancestry) &&
    ...
    function siblingLeft(prop, index) {
        return prop.slice(0, index).some(function (precedingSibling) {
            return (0, match_1.findMatches)(precedingSibling, selector.left, ancestry);
        });
    }
    function siblingRight(prop, index) {
        return prop.slice(index, prop.length).some(function (followingSibling) {
            return (0, match_1.findMatches)(followingSibling, selector.right, ancestry);
        });
    }
}
...
function adjacent(node, selector, ancestry) {
    ...
    function adjacentLeft(prop, index) {
        return index > 0 && (0, match_1.findMatches)(prop[index - 1], selector.left, ancestry);
    }
    function adjacentRight(prop, index) {
        return index < prop.length - 1 && (0, match_1.findMatches)(prop[index + 1], selector.right, ancestry);
    }
}
