...
//#region lib/rules/no-missing-placeholders.ts
const rule = {
	meta: {
		type: "problem",
		docs: {
		...
		fixable: void 0,
		schema: [],
		messages: { placeholderDoesNotExist: "The placeholder {{{{missingKey}}}} is missing (must provide it in the report's `data` object)." }
	},
	create(context) {
		...
		return {
			...
			CallExpression(node) {
				...
				if (node.callee.type === "MemberExpression" && contextIdentifiers.has(node.callee.object) && node.callee.property.type === "Identifier" && node.callee.property.name === "report") {
					...
					for (const { message, messageId, data } of reportMessagesAndDataArray.filter((obj) => obj.message)) {
						if (!message) continue;
						...
						if ((message?.type === "Literal" && typeof message.value === "string" || messageStaticValue && typeof messageStaticValue.value === "string") && (!data || data.type === "ObjectExpression")) {
							...
							let match;
							const messageText = message.value || messageStaticValue.value;
							while (match = PLACEHOLDER_MATCHER.exec(messageText)) if (!(data && data.properties.find((prop) => getKeyName(prop) === match[1]))) context.report({
...
//#endregion
export { no_missing_placeholders_default as default };