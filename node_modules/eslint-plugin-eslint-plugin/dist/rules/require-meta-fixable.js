import { evaluateObjectProperties, getContextIdentifiers, getKeyName, getRuleInfo } from "../utils.js";
import { getStaticValue } from "@eslint-community/eslint-utils";

//#region lib/rules/require-meta-fixable.ts
const rule = {
	meta: {
		type: "problem",
		docs: {
		...
		schema: [{
			type: "object",
			properties: { catchNoFixerButFixableProperty: {
				type: "boolean",
				default: false,
	...
	create(context) {
		...
		return {
			...
			CallExpression(node) {
				if (node.callee.type === "MemberExpression" && contextIdentifiers.has(node.callee.object) && node.callee.property.type === "Identifier" && node.callee.property.name === "report" && (node.arguments.length > 4 || node.arguments.length === 1 && evaluateObjectProperties(node.arguments[0], scopeManager).some((prop) => getKeyName(prop) === "fix"))) usesFixFunctions = true;
			},
			"Program:exit"(ast) {
				...
				if (metaFixableProp) {
					...
					if (usesFixFunctions && (typeof staticValue.value !== "string" || !["code", "whitespace"].includes(staticValue.value))) context.report({
						node: metaFixableProp.value,
						messageId: "missing"
					});
					else if (catchNoFixerButFixableProperty && !usesFixFunctions && typeof staticValue.value === "string" && ["code", "whitespace"].includes(staticValue.value)) context.report({
						node: metaFixableProp.value,
						messageId: "noFixerButFixableValue"
					});
				} else if (!metaFixableProp && usesFixFunctions) context.report({
...
//#endregion
export { require_meta_fixable_default as default };