import { getContextIdentifiers, getMetaSchemaNode, getMetaSchemaNodeProperty, getRuleInfo, insertProperty, isUndefinedIdentifier } from "../utils.js";

//#region lib/rules/require-meta-schema.ts
const rule = {
	meta: {
		type: "suggestion",
		docs: {
		...
		schema: [{
			type: "object",
			properties: { requireSchemaPropertyWhenOptionless: {
				type: "boolean",
				default: true,
	...
	create(context) {
		...
		return {
			Program(ast) {
				...
				if (schemaProperty.type === "ArrayExpression" && schemaProperty.elements.length === 0 || schemaProperty.type === "ObjectExpression" && schemaProperty.properties.length === 0) hasEmptySchema = true;
				if (schemaProperty.type === "Literal" || isUndefinedIdentifier(schemaProperty)) context.report({
					node: schemaProperty,
					messageId: "wrongType"
				});
			},
			"Program:exit"() {
				if (!schemaNode && requireSchemaPropertyWhenOptionless) context.report({
					node: metaNode || ruleInfo.create,
					messageId: "missing",
					suggest: !isUsingOptions && metaNode && metaNode.type === "ObjectExpression" ? [{
						messageId: "addEmptySchema",
						fix(fixer) {
							return insertProperty(fixer, metaNode, "schema: []", sourceCode);
						}
					}] : []
				});
			},
...
//#endregion
export { require_meta_schema_default as default };