(function (global, factory) {
    ...
    class Parser {
        constructor(grammar, lexer, baseParser, { externalParsers } = {}) {
    ...
    const functionPropertyParslet = composeParslet({
        ...
        accept: (type, next) => type === 'new' && next === '(' ||
            ...
            while (true) {
                if (currentText.startsWith('${')) {
                    ...
                    while (true) {
                        ...
                        try {
                            templateParser = new Parser(typescriptGrammar, Lexer.create(parser.lexer.lexerRules, snipped));
     ...
     * @param mode
     */
    function parse(expression, mode, { computedPropertyParser } = {}) {
        switch (mode) {
            case 'closure':
            ...
            case 'typescript':
                return (new Parser(typescriptGrammar, Lexer.create(rules, expression), undefined, computedPropertyParser === undefined ? undefined : {
                    externalParsers: {
    ...
    function stringifyRules({ computedPropertyStringifier } = {}) {
        return {
            ...
            JsdocTypeCallSignature: (result, transform) => `(${result.parameters.map(transform).join(', ')}): ${transform(result.returnType)}`,
            JsdocTypeConstructorSignature: (result, transform) => `new (${result.parameters.map(transform).join(', ')}): ${transform(result.returnType)}`,
            JsdocTypeMethodSignature: (result, transform) => {
                const quote = result.meta.quote === 'double'
                    ...
                    : result.meta.quote === 'single'
                        ? "'"
                        : '';
                return `${quote}${result.name}${quote}(${result.parameters.map(transform).join(', ')}): ${transform(result.returnType)}`;
            },
        ...
        result.literals.slice(0, -1).map((literal, idx) => literal.replace(/`/gu, '\\`') + '${' + transform(result.interpolations[idx]) + '}').join('') + result.literals.slice(-1)[0].replace(/`/gu, '\\`')}\``),
            ...
            JsdocTypeComputedMethod: (result, transform) => {
                if (result.value.type.startsWith('JsdocType')) {
                    return `[${transform(result.value)}]${result.optional ? '?' : ''}(${result.parameters.map(transform).join(', ')}): ${transform(result.returnType)}`;
                }
                else {
                    if (computedPropertyStringifier === undefined) {
                        throw new Error('Must have a computed property stringifier');
                    }
                    return `[${computedPropertyStringifier(result.value).replace(/;$/u, '')}](${result.parameters.map(transform).join(', ')}): ${transform(result.returnType)}`;
                }
            }
        };
    }
