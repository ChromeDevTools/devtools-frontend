...
async function compare(golden: string, generated: string, maximumDiffThreshold: number) {
  ...
  try {
    ...
    throw ScreenshotError.fromError(assertionError, generated, diffPath, golden);
  }
}
...
export class ScreenshotError extends Error {
  ...
  static readonly SUMMARY_LENGTH_CUTOFF = 3950;
  readonly goldenImg?: string;
  readonly diffImg?: string;
  readonly generatedImg: string;

  private constructor(
      generatedImgPath: string, message?: string, cause?: Error, diffImgPath?: string, goldenImgPath?: string) {
    message = message ?? cause?.message ?? '';
    super(message);
    this.cause = cause;
    this.generatedImg = this.stashArtifact(generatedImgPath, 'actual');
    if (diffImgPath) {
      this.diffImg = this.stashArtifact(diffImgPath, 'diff');
    }
    if (goldenImgPath) {
      this.goldenImg = this.stashArtifact(goldenImgPath, 'expected');
    }
  }

  static fromMessage(message: string, generatedImgPath: string) {
    return new ScreenshotError(generatedImgPath, message);
  }

  static fromError(error: Error, generatedImgPath: string, diffImgPath: string, goldenImgPath: string) {
    return new ScreenshotError(generatedImgPath, undefined, error, diffImgPath, goldenImgPath);
  }
  ...
  toMiloArtifacts(): [ArtifactGroup, string] {
    if (this.goldenImg && this.diffImg) {
      return [
        {
          'actual_image': {filePath: this.generatedImg},
          'expected_image': {filePath: this.goldenImg},
          'image_diff': {filePath: this.diffImg},
        },
    ...
    return [
      {
        'generated': {filePath: this.generatedImg},
      },
      // TODO(liviurau): embed image once Milo supports it
      `<pre>${
          this.message.slice(0, ScreenshotError.SUMMARY_LENGTH_CUTOFF)}</pre><p>Screenshot generated (see below)</p>`,
    ];
  }
   ...
   * at resultdb publication time.
   */
  private stashArtifact(originalFile: string, tag: string): string {
