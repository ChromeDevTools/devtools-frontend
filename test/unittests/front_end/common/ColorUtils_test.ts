...
import {blendColors, contrastRatio, luminance, rgbaToHsla} from '../../../../front_end/common/ColorUtils.js';
...
it('is able to convert RGBA to HSLA', () => {
  const result = rgbaToHsla([0.5, 0.5, 0.5, 0.5]);
...
it('is able to return the luminance of an RGBA value with the RGB values more than 0.03928', () => {
  const lum = luminance([0.5, 0.5, 0.5, 0.5]);
...
it('is able to return the luminance of an RGBA value with the RGB values less than 0.03928', () => {
  const lum = luminance([0.03927, 0.03927, 0.03927, 0.5]);
...
it('is able to calculate the contrast ratio between two colors', () => {
  ...
  const secondColor = [0, 0, 1, 1];
  assert.strictEqual(
      contrastRatio(firstColor, secondColor), 2.148936170212766, 'contrast ratio was not calculated correctly');
});
